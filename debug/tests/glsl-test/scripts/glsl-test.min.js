...
d=null,g=$("#canvas-location"),h=0;for(let a=0;a<c.length;a++){console.log(`Starting test ${a}: ${c[a]}`);var b=c[a];try{try{d.outputCanvasContainer.parentNode.remove(),g.insertAdjacentHTML("afterend",'\n\t\t\t\t\t<canvas id="output-canvas" class="output-canvas"></canvas>\n\t\t\t\t')}catch(k){}let e={renderer:"gpu",shader:`
			...
			`,canvasWidth:1,canvasHeight:1};d=new Wilson($("#output-canvas"),e);d.render.drawFrame();var f=0===d.render.getPixelData()[0]?0:1}catch(e){f=2}b=f;0===b&&h++;if(1===b){console.error(`Test ${a} returned false: ${c[a]}`);passedEverything=!1;break}}Page.show()}();
