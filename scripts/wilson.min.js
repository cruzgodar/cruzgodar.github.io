...
this.shaderProgram=this.parent.gl.createProgram();this.parent.gl.attachShader(this.shaderProgram,a);this.parent.gl.attachShader(this.shaderProgram,c);this.parent.gl.linkProgram(this.shaderProgram);this.parent.gl.getProgramParameter(this.shaderProgram,this.parent.gl.LINK_STATUS)||(console.error(`[Wilson] Couldn't link shader program: ${this.parent.gl.getShaderInfoLog(shader)}`),this.parent.gl.deleteProgram(this.shaderProgram));this.parent.gl.useProgram(this.shaderProgram);a=this.parent.gl.createBuffer();
this.parent.gl.bindBuffer(this.parent.gl.ARRAY_BUFFER,a);this.parent.gl.bufferData(this.parent.gl.ARRAY_BUFFER,new Float32Array([-1,-1,0,-1,1,0,1,-1,0,1,1,0]),this.parent.gl.STATIC_DRAW);this.shaderProgram.positionAttribute=this.parent.gl.getAttribLocation(this.shaderProgram,"position");this.parent.gl.enableVertexAttribArray(this.shaderProgram.positionAttribute);this.parent.gl.vertexAttribPointer(this.shaderProgram.positionAttribute,3,this.parent.gl.FLOAT,!1,0,0);this.parent.gl.viewport(0,0,this.parent.canvasWidth,
this.parent.canvasHeight);this.parent.gl.pixelStorei(this.parent.gl.UNPACK_ALIGNMENT,1);this.parent.gl.pixelStorei(this.parent.gl.UNPACK_FLIP_Y_WEBGL,1);this.texture=this.parent.gl.createTexture();this.parent.gl.bindTexture(this.parent.gl.TEXTURE_2D,this.texture);this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,this.parent.gl.TEXTURE_WRAP_S,this.parent.gl.CLAMP_TO_EDGE);this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,this.parent.gl.TEXTURE_WRAP_T,this.parent.gl.CLAMP_TO_EDGE);this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,
this.parent.gl.TEXTURE_MAG_FILTER,this.parent.gl.NEAREST);this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,this.parent.gl.TEXTURE_MIN_FILTER,this.parent.gl.NEAREST);this.parent.gl.disable(this.parent.gl.DEPTH_TEST)},loadNewShader(b){function a(d,e,f){e=d.createShader(e);d.shaderSource(e,f);d.compileShader(e);d.getShaderParameter(e,d.COMPILE_STATUS)||(console.log(`[Wilson] Couldn't load shader: ${d.getProgramInfoLog(shaderProgram)}`),d.deleteShader(e));return e}this.parent.gl=this.parent.canvas.getContext("webgl");
var c=a(this.parent.gl,this.parent.gl.VERTEX_SHADER,"\n\t\t\t\tattribute vec3 position;\n\t\t\t\tvarying vec2 uv;\n\n\t\t\t\tvoid main(void)\n\t\t\t\t{\n\t\t\t\t\tgl_Position = vec4(position, 1.0);\n\n\t\t\t\t\t//Interpolate quad coordinates in the fragment shader.\n\t\t\t\t\tuv = position.xy;\n\t\t\t\t}\n\t\t\t");b=a(this.parent.gl,this.parent.gl.FRAGMENT_SHADER,b);this.shaderProgram=this.parent.gl.createProgram();this.parent.gl.attachShader(this.shaderProgram,c);this.parent.gl.attachShader(this.shaderProgram,
b);this.parent.gl.linkProgram(this.shaderProgram);this.parent.gl.getProgramParameter(this.shaderProgram,this.parent.gl.LINK_STATUS)||(console.log(`[Wilson] Couldn't link shader program: ${this.gl.getShaderInfoLog(shader)}`),this.parent.gl.deleteProgram(this.shaderProgram));this.parent.gl.useProgram(this.shaderProgram);c=this.parent.gl.createBuffer();this.parent.gl.bindBuffer(this.parent.gl.ARRAY_BUFFER,c);this.parent.gl.bufferData(this.parent.gl.ARRAY_BUFFER,new Float32Array([-1,-1,0,-1,1,0,1,-1,
0,1,1,0]),this.parent.gl.STATIC_DRAW);this.shaderProgram.positionAttribute=this.parent.gl.getAttribLocation(this.shaderProgram,"position");this.parent.gl.enableVertexAttribArray(this.shaderProgram.positionAttribute);this.parent.gl.vertexAttribPointer(this.shaderProgram.positionAttribute,3,this.parent.gl.FLOAT,!1,0,0);this.parent.gl.viewport(0,0,this.parent.canvasWidth,this.parent.canvasHeight);this.shaderPrograms.push(this.shaderProgram)},createFramebufferTexturePair(b=this.parent.gl.FLOAT){let a=
this.parent.gl.createFramebuffer(),c=this.parent.gl.createTexture();this.parent.gl.bindTexture(this.parent.gl.TEXTURE_2D,c);this.parent.gl.texImage2D(this.parent.gl.TEXTURE_2D,0,this.parent.gl.RGBA,this.parent.canvasWidth,this.parent.canvasHeight,0,this.parent.gl.RGBA,b,null);this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,this.parent.gl.TEXTURE_MAG_FILTER,this.parent.gl.NEAREST);this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,this.parent.gl.TEXTURE_MIN_FILTER,this.parent.gl.NEAREST);
this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,this.parent.gl.TEXTURE_WRAP_S,this.parent.gl.CLAMP_TO_EDGE);this.parent.gl.texParameteri(this.parent.gl.TEXTURE_2D,this.parent.gl.TEXTURE_WRAP_T,this.parent.gl.CLAMP_TO_EDGE);this.parent.gl.disable(this.parent.gl.DEPTH_TEST);this.parent.gl.bindFramebuffer(this.parent.gl.FRAMEBUFFER,a);this.parent.gl.framebufferTexture2D(this.parent.gl.FRAMEBUFFER,this.parent.gl.COLOR_ATTACHMENT0,this.parent.gl.TEXTURE_2D,c,0);this.framebuffers.push({framebuffer:a,
texture:c})},initUniforms(b,a=-1){if(-1===a)for(a=0;a<b.length;a++)this.parent.uniforms[b[a]]=this.parent.gl.getUniformLocation(this.shaderProgram,b[a]);else for(let c=0;c<b.length;c++)"undefined"===typeof this.parent.uniforms[b[c]]&&(this.parent.uniforms[b[c]]={}),this.parent.uniforms[b[c]][a]=this.parent.gl.getUniformLocation(this.shaderPrograms[a],b[c])},getPixelData(){let b=new Uint8Array(this.parent.canvasWidth*this.parent.canvasHeight*4);this.parent.gl.readPixels(0,0,this.parent.canvasWidth,
this.parent.canvasHeight,this.parent.gl.RGBA,this.parent.gl.UNSIGNED_BYTE,b);return b}};draggables={parent:null,container:null,containerWidth:null,containerHeight:null,restrictedWidth:null,restrictedHeight:null,draggables:[],worldCoordinates:[],autoAddContainer:!0,numDraggables:0,static:!1,draggableRadius:12,activeDraggable:-1,lastActiveDraggable:-1,mouseX:0,mouseY:0,mousedownCallback:null,mouseupCallback:null,mousemoveCallback:null,touchstartCallback:null,touchendCallback:null,touchmoveCallback:null,
init(){if(0===document.querySelectorAll("#wilson-draggables-style").length){var b=document.createElement("style");b.textContent="\n\t\t\t\t\t.wilson-output-canvas-container\n\t\t\t\t\t{\n\t\t\t\t\t\tposition: relative;\n\t\t\t\t\t\twidth: fit-content;\n\t\t\t\t\t}\n\n\t\t\t\t\t.wilson-output-canvas-container.wilson-fullscreen\n\t\t\t\t\t{\n\t\t\t\t\t\twidth: 100%;\n\t\t\t\t\t}\n\t\t\t\t\t\n\t\t\t\t\t.wilson-draggables-container\n\t\t\t\t\t{\n\t\t\t\t\t\tposition: absolute;\n\t\t\t\t\t\t\n\t\t\t\t\t\t-webkit-user-select: none;\n\t\t\t\t\t\tuser-select: none;\n\t\t\t\t\t}\n\n\t\t\t\t\t.wilson-draggable\n\t\t\t\t\t{\n\t\t\t\t\t\tposition: absolute;\n\t\t\t\t\t\t\n\t\t\t\t\t\twidth: 20px;\n\t\t\t\t\t\theight: 20px;\n\t\t\t\t\t\t\n\t\t\t\t\t\tleft: 0;\n\t\t\t\t\t\ttop: 0;\n\t\t\t\t\t\t\n\t\t\t\t\t\tbackground-color: rgb(255, 255, 255);\n\t\t\t\t\t\tborder: 2px solid rgb(64, 64, 64);\n\t\t\t\t\t\tborder-radius: 50%;\n\t\t\t\t\t\t\n\t\t\t\t\t\ttouch-action: none;\n\t\t\t\t\t\t-webkit-touch-callout: none;\n\t\t\t\t\t\t-webkit-user-select: none;\n\t\t\t\t\t\tuser-select: none;\n\t\t\t\t\t\t\n\t\t\t\t\t\tcursor: pointer;\n\t\t\t\t\t\t\n\t\t\t\t\t\ttransition: width .125s ease-in-out, height .125s ease-in-out, top .125s ease-in-out, left .125s ease-in-out;\n\t\t\t\t\t}\n\n\t\t\t\t\t.wilson-draggable:active\n\t\t\t\t\t{\n\t\t\t\t\t\twidth: 16px;\n\t\t\t\t\t\theight: 16px;\n\t\t\t\t\t\t\n\t\t\t\t\t\tleft: 2px;\n\t\t\t\t\t\ttop: 2px;\n\t\t\t\t\t}\n\t\t\t\t";
