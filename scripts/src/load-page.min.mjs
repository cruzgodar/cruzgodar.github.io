import{fadeUpIn,fadeDownIn,fadeLeftIn,fadeRightIn,fadeIn}from"./animation.min.mjs";import{setUpBanner,bannerElement,bannerOpacity}from"./banners.min.mjs";import{setUpTextButtons,setUpNavButtons,setUpDropdowns}from"./buttons.min.mjs";import{setUpCards}from"./cards.min.mjs";import{addHoverEvent,setUpHoverEvents,setUpFocusEvents}from"./hover-events.min.mjs";import{onResize}from"./layout.min.mjs";import{typesetMath}from"./math.min.mjs";import{redirect,navigationTransitionType}from"./navigation.min.mjs";import{siteSettings,toggleDarkTheme,revertTheme,condenseApplet}from"./settings.min.mjs";let headerElement=null;async function loadPage(){window.dispatchEvent(new Event("scroll")),Page.element=document.body.querySelector(".page"),$=e=>Page.element.querySelector(e),$$=e=>Page.element.querySelectorAll(e);try{document.head.querySelector("title").textContent=Site.sitemap[Page.url].title}catch(e){}setUpBanner(),loadCustomStyle(),loadCustomScripts(),equalizeAppletColumns(),setLinks(),disableLinks(),setUpHoverEvents(),setUpTextButtons(),setUpNavButtons(),setUpDropdowns(),typesetMath(),setUpCards(),onResize(),Page.backgroundColorChanged=!1,revertTheme(),siteSettings.condensedApplets&&"/applets/"===Site.sitemap[Page.url].parent&&condenseApplet(),setTimeout(()=>setUpFocusEvents(),50)}async function showPage(){await new Promise((e,t)=>setTimeout(e,10)),await fadeInPage()}function loadCustomStyle(){fetch(Page.url+"style/index."+(window.DEBUG?"css":"min.css")).then(e=>e.text()).then(e=>{var t=document.createElement("style");t.textContent=e,t.classList.add("temporary-style"),document.head.insertBefore(t,document.head.firstChild)}).catch(e=>{})}function loadCustomScripts(){import(Page.url+"scripts/index."+(window.DEBUG?"mjs":"min.mjs")).then(e=>e.load()).catch(()=>{setTimeout(()=>showPage(),1)})}function addHeader(){document.body.firstChild.insertAdjacentHTML("beforebegin",`
