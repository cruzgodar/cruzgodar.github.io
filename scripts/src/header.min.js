import anime from"../anime.min.js";import{Checkbox}from"./checkboxes.min.js";import{addHoverEvent,addHoverEventWithScale}from"./hoverEvents.min.js";import{redirect}from"./navigation.min.js";import{siteSettings,toggleDarkTheme,toggleIncreaseContrast,toggleReduceMotion}from"./settings.min.js";let headerElement,accessibilityTooltipElement,reduceMotionCheckbox,increaseContrastCheckbox,accessibilityDialogOpen=!1;function addHeader(){document.body.firstChild.insertAdjacentHTML("beforebegin",`
	...
	`),setTimeout(()=>{var e=document.body.querySelector("#header-logo img"),e=(e.style.width=e.getBoundingClientRect().height+"px",document.body.querySelectorAll("#header-logo, #header-links a").forEach(link=>{addHoverEvent(link);const i=link.getAttribute("href");link.setAttribute("href","/index.html?page="+encodeURIComponent(i)),link.addEventListener("click",e=>{e.preventDefault(),redirect({url:i,inNewTab:e.metaKey})})}),document.body.querySelector("#header-theme-button")),e=(addHoverEvent(e),e.addEventListener("click",e=>{e.preventDefault(),toggleDarkTheme({})}),document.body.querySelector("#header-accessibility-button"));addHoverEvent(e),e.addEventListener("click",e=>{e.preventDefault(),(accessibilityDialogOpen?hideAccessibilityDialog:showAccessibilityDialog)()}),accessibilityTooltipElement=document.body.querySelector("#accessibility-tooltip"),document.documentElement.addEventListener("pointerdown",e=>{accessibilityDialogOpen&&!e.target.classList.contains("keep-accessibility-dialog-open")&&hideAccessibilityDialog()}),reduceMotionCheckbox=new Checkbox({element:document.body.querySelector("#reduce-motion-checkbox"),name:"Reduce motion",checked:siteSettings.reduceMotion,onInput:()=>toggleReduceMotion()}),addHoverEventWithScale({element:reduceMotionCheckbox.element.parentNode,scale:1.1,addBounceOnTouch:!0}),increaseContrastCheckbox=new Checkbox({element:document.body.querySelector("#increase-contrast-checkbox"),name:"Increase contrast",checked:siteSettings.increaseContrast,onInput:()=>toggleIncreaseContrast({})}),addHoverEventWithScale({element:increaseContrastCheckbox.element.parentNode,scale:1.1,addBounceOnTouch:!0}),headerElement=document.body.querySelector("#header")})}async function showAccessibilityDialog(){accessibilityDialogOpen||(accessibilityDialogOpen=!0,accessibilityTooltipElement.style.transform=siteSettings.reduceMotion?"scale(1)":"translateX(8px) translateY(-8px) scale(.95)",accessibilityTooltipElement.style.display="block",await anime({targets:accessibilityTooltipElement,opacity:1,scale:1,translateX:0,translateY:0,duration:150,easing:"easeOutQuad"}).finished)}async function hideAccessibilityDialog(){accessibilityDialogOpen&&(accessibilityDialogOpen=!1,await anime({targets:accessibilityTooltipElement,opacity:0,scale:siteSettings.reduceMotion?1:.95,translateX:siteSettings.reduceMotion?0:8,translateY:siteSettings.reduceMotion?0:-8,duration:150,easing:"easeOutQuad"}).finished,accessibilityTooltipElement.style.display="none")}export{headerElement,reduceMotionCheckbox,increaseContrastCheckbox,addHeader};