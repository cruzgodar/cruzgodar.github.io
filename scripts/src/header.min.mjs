import{addHoverEvent}from"./hover-events.min.mjs";import{redirect}from"./navigation.min.mjs";import{toggleDarkTheme}from"./settings.min.mjs";let headerElement=null;function addHeader(){document.body.firstChild.insertAdjacentHTML("beforebegin",`
	...
	`),setTimeout(()=>{var imageElement=document.body.querySelector("#header-logo img"),imageElement=(imageElement.style.width=imageElement.getBoundingClientRect().height+"px",document.body.querySelectorAll("#header-logo, #header-links a").forEach(link=>{addHoverEvent(link);const href=link.getAttribute("href");link.setAttribute("href","/index.html?page="+encodeURIComponent(href)),link.addEventListener("click",e=>{e.preventDefault(),redirect({url:href})})}),document.body.querySelector("#header-theme-button"));addHoverEvent(imageElement),imageElement.addEventListener("click",()=>toggleDarkTheme({})),headerElement=document.body.querySelector("#header")})}export{headerElement,addHeader};