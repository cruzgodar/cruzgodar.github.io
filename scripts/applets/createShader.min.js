import{doubleEncodingGlsl}from"../src/complexGlsl.min.js";import{getFloatGlsl,getVectorGlsl}from"./applet.min.js";function getComputeShadowIntensityGlsl({useShadows,useSoftShadows,getGeodesicGlsl}){return useShadows&&useSoftShadows?`
		...
		`:""}function getComputeReflectionsGlsl({useReflections,useOppositeLight,oppositeLightBrightness,ambientLight,getGeodesicGlsl,useBloom,useShadows}){return useReflections?`
		...
		`:""}function getComputeShadingGlsl({useForDepthBuffer,useOppositeLight,oppositeLightBrightness,ambientLight,useShadows,useReflections,stepFactor}){return useForDepthBuffer?`
			vec3 computeShading(
				vec3 pos,
				float epsilon,
				float distanceToScene,
	...
	`}function getRaymarchGlsl({useForDepthBuffer,stepFactor,getGeodesicGlsl,useBloom}){var e=useForDepthBuffer?"gl_FragColor = encodeFloat(clipDistance); return vec3(0.0);":useBloom?"return mix(fogColor, vec3(1.0), computeBloom(rayDirectionVec));":"return fogColor;";return`
	...
	`}function getMainFunctionGlsl({useForDepthBuffer,useFor3DPrinting}){return useForDepthBuffer?`
			void main(void)
			{
				raymarch(
					imagePlaneCenterPos
	...
	`}function createShader({distanceEstimatorGlsl,getColorGlsl,getReflectivityGlsl,getGeodesicGlsl,addGlsl,useForDepthBuffer,useShadows,useSoftShadows,useReflections,useOppositeLight,oppositeLightBrightness,ambientLight,useBloom,bloomPower,stepFactor,useFor3DPrinting,uniformsGlsl,lightPos,lightBrightness,clipDistance,maxMarches,maxShadowMarches,maxReflectionMarches,fogColor,fogScaling}){var e=getComputeShadowIntensityGlsl({useShadows:useShadows,useSoftShadows:useSoftShadows,getGeodesicGlsl:getGeodesicGlsl}),t=getComputeReflectionsGlsl({useReflections:useReflections,useOppositeLight:useOppositeLight,oppositeLightBrightness:oppositeLightBrightness,ambientLight:ambientLight,getGeodesicGlsl:getGeodesicGlsl,useBloom:useBloom,useShadows:useShadows}),o=getComputeShadingGlsl({useForDepthBuffer:useForDepthBuffer,useOppositeLight:useOppositeLight,oppositeLightBrightness:oppositeLightBrightness,ambientLight:ambientLight,useShadows:useShadows,useReflections:useReflections,stepFactor:stepFactor}),s=getRaymarchGlsl({useForDepthBuffer:useForDepthBuffer,stepFactor:stepFactor,getGeodesicGlsl:getGeodesicGlsl,useBloom:useBloom}),i=getMainFunctionGlsl({useForDepthBuffer:useForDepthBuffer,useFor3DPrinting:useFor3DPrinting}),a=useBloom?`
