import{getFloatGlsl,getVectorGlsl}from"./applet.min.js";function getComputeShadowIntensityGlsl({useShadows,useSoftShadows,getGeodesicGlsl,stepFactor}){return useShadows&&useSoftShadows?`
		...
		`:""}function getComputeShadingGlsl({useOppositeLight,oppositeLightBrightness,ambientLight,useShadows,useGradientCorrectedOcclusion}){return`
	...
	`}function getRaymarchGlsl({getGeodesicGlsl,stepFactor,overstepFactor,coneMarchingScale}){return`
		void raymarch(
			vec3 rayOrigin,
	...
	`}function getMainFunctionGlsl({useFor3DPrinting,includeDepthData,useBloom,useShadows,useReflections,surfaceNormalEpsilonFactor}){var e,t;return useFor3DPrinting?`
			void main(void)
			{
		...
		{
			reflectionColor = ${useBloom?"mix(fogColor, vec3(1.0), computeBloom(reflectedDirection))":"fogColor"};
		}
	...
	`:"",`
		void main(void)
		{
			...
			if (t > clipDistance)
			{
				gl_FragColor = vec4(${useBloom?"mix(fogColor, vec3(1.0), computeBloom(rayDirectionVec))":"fogColor"}, ${e=includeDepthData?"t":"1.0"});
				return;
			}
	...
	`)}function createShader({distanceEstimatorGlsl,getColorGlsl,getReflectivityGlsl,getGeodesicGlsl,addGlsl,includeDepthData,useShadows,useSoftShadows,useReflections,useOppositeLight,useGradientCorrectedOcclusion,oppositeLightBrightness,ambientLight,useBloom,bloomPower,stepFactor,overstepFactor,surfaceNormalEpsilonFactor,useFor3DPrinting,coneMarchingScale,aoSamples,aoStrength,uniformsGlsl,lightPos,lightBrightness,clipDistance,maxMarches,maxShadowMarches,fogColor,fogScaling}){var e=getComputeShadowIntensityGlsl({useShadows:useShadows,useSoftShadows:useSoftShadows,getGeodesicGlsl:getGeodesicGlsl,stepFactor:stepFactor}),t=getComputeShadingGlsl({useOppositeLight:useOppositeLight,oppositeLightBrightness:oppositeLightBrightness,ambientLight:ambientLight,useShadows:useShadows,useGradientCorrectedOcclusion:useGradientCorrectedOcclusion}),o=getRaymarchGlsl({getGeodesicGlsl:getGeodesicGlsl,stepFactor:stepFactor,overstepFactor:overstepFactor,coneMarchingScale:coneMarchingScale}),a=getMainFunctionGlsl({useFor3DPrinting:useFor3DPrinting,includeDepthData:includeDepthData,useBloom:useBloom,useShadows:useShadows,useReflections:useReflections,surfaceNormalEpsilonFactor:surfaceNormalEpsilonFactor}),i=useBloom?`
	...
	`:"";return`
		...
		${useReflections?`float getReflectivity(vec3 pos)
		{
			${getReflectivityGlsl}
		}`:""}
		...
		${a}
	`}function createConeMarchingShader({distanceEstimatorGlsl,addGlsl,stepFactor,uniformsGlsl,clipDistance,maxMarches,coneMarchingScale,isFirstScale}){var e=`
		void raymarch(
			vec3 rayOrigin,
			vec3 rayDirectionVec,
	...
	`}export{createShader,createConeMarchingShader};