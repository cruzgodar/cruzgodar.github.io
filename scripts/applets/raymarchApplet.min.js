import anime from"../anime.min.js";import{doubleEncodingGlsl,loadGlsl}from"../src/complexGlsl.min.js";import{sleep}from"../src/utils.min.js";import{WilsonCPU,WilsonGPU}from"../wilson.min.js";import{AnimationFrameApplet}from"./animationFrameApplet.min.js";import{getFloatGlsl,getVectorGlsl,tempShader}from"./applet.min.js";const edgeDetectShader=`
...
`;class RaymarchApplet extends AnimationFrameApplet{movingSpeed=.1;moveVelocity=[0,0,0];moveFriction=.96;moveStopThreshhold=.01;lastTimestamp=-1;theta=0;phi=0;worldSize=2.5;resolution=500;maxMarches;maxShadowMarches;maxReflectionMarches;clipDistance;imagePlaneCenterPos=[0,0,0];forwardVec=[0,0,0];rightVec=[0,0,0];upVec=[0,0,0];focalLengthFactor;cameraPos;lightPos;lightBrightness;useOppositeLight;oppositeLightBrightness;ambientLight;bloomPower;fogColor;fogScaling;stepFactor;epsilonScaling;minEpsilon;useShadows;useSoftShadows;useReflections;useBloom;useAntialiasing;useFor3DPrinting;uniforms={};lockZ;speedFactor=2;fovFactor=1;lockedOnOrigin;distanceFromOrigin=1;distanceEstimatorGlsl;getColorGlsl;uniformsGlsl;getReflectivityGlsl;getGeodesicGlsl;addGlsl;constructor({canvas,shader,resolution=500,distanceEstimatorGlsl,getColorGlsl,uniformsGlsl="",getReflectivityGlsl="return 0.2;",getGeodesicGlsl=(pos,dir)=>pos+" + t * "+dir,addGlsl="",uniforms={},theta=0,phi=Math.PI/2,stepFactor=.99,epsilonScaling=1.75,minEpsilon=3e-7,maxMarches=128,maxShadowMarches=128,maxReflectionMarches=128,clipDistance=1e3,focalLengthFactor=2.5,cameraPos=[0,0,0],lockedOnOrigin=!0,lockZ,lightPos=[50,70,100],lightBrightness=1,useOppositeLight=!0,oppositeLightBrightness=.5,ambientLight=.25,bloomPower=1,fogColor=[0,0,0],fogScaling=.05,useShadows=!1,useSoftShadows=!0,useReflections=!1,useBloom=!0,useAntialiasing=!1,useFor3DPrinting=!1}){super(canvas),this.resolution=resolution,this.theta=theta,this.phi=phi,this.stepFactor=stepFactor,this.epsilonScaling=epsilonScaling,this.minEpsilon=minEpsilon,this.maxMarches=maxMarches,this.maxShadowMarches=maxShadowMarches,this.maxReflectionMarches=maxReflectionMarches,this.clipDistance=clipDistance,this.focalLengthFactor=focalLengthFactor,this.cameraPos=cameraPos,this.lockedOnOrigin=lockedOnOrigin,this.worldSize=this.lockedOnOrigin?2.5:1.5,this.lockZ=lockZ,this.lightPos=lightPos,this.lightBrightness=lightBrightness,this.useOppositeLight=useOppositeLight,this.oppositeLightBrightness=oppositeLightBrightness,this.ambientLight=ambientLight,this.bloomPower=bloomPower,this.fogColor=fogColor,this.fogScaling=fogScaling,this.useShadows=useShadows,this.useSoftShadows=useSoftShadows,this.useReflections=useReflections,this.useBloom=useBloom,this.useAntialiasing=useAntialiasing,this.useFor3DPrinting=useFor3DPrinting,this.uniformsGlsl=`
		...
		`:"",s=useForDepthBuffer?`
			...
			${a}
		`}reloadShader({distanceEstimatorGlsl,getColorGlsl,getReflectivityGlsl,addGlsl,useAntialiasing=this.useAntialiasing,useForDepthBuffer}={}){var e;this.useAntialiasing=useAntialiasing,this.wilson.loadShader({id:"draw",shader:this.createShader({distanceEstimatorGlsl:distanceEstimatorGlsl,getColorGlsl:getColorGlsl,getReflectivityGlsl:getReflectivityGlsl,addGlsl:addGlsl,useForDepthBuffer:useForDepthBuffer}),uniforms:this.uniforms}),this.useAntialiasing&&(this.wilson.loadShader({id:"edgeDetect",shader:edgeDetectShader,uniforms:{stepSize:[1/this.wilson.canvasWidth,1/this.wilson.canvasHeight]}}),e=this.createShader({distanceEstimatorGlsl:distanceEstimatorGlsl,getColorGlsl:getColorGlsl,getReflectivityGlsl:getReflectivityGlsl,addGlsl:addGlsl,useForDepthBuffer:useForDepthBuffer,antialiasing:!0}),this.wilson.loadShader({id:"antialias",shader:e,uniforms:{...this.uniforms,stepSize:[2/(3*this.wilson.canvasWidth),2/(3*this.wilson.canvasHeight)]}}),this.createTextures()),this.calculateVectors(),this.needNewFrame=!0}createTextures(){this.wilson.createFramebufferTexturePair({id:"0",textureType:"float"}),this.wilson.createFramebufferTexturePair({id:"1",textureType:"float"}),this.wilson.useFramebuffer(null),this.wilson.useTexture("0"),this.wilson.setUniforms({stepSize:[1/this.wilson.canvasWidth,1/this.wilson.canvasHeight]},"edgeDetect"),this.wilson.setUniforms({stepSize:[2/(3*this.wilson.canvasWidth),2/(3*this.wilson.canvasHeight)]},"antialias"),this.wilson.useShader("draw")}setUniforms(uniforms){this.uniforms={...this.uniforms,...uniforms},this.wilson.setUniforms(uniforms,"draw"),this.useAntialiasing&&this.wilson.setUniforms(uniforms,"antialias"),this.needNewFrame=!0}calculateVectors(){this.useFor3DPrinting||(this.forwardVec=[Math.cos(this.theta)*Math.sin(this.phi),Math.sin(this.theta)*Math.sin(this.phi),Math.cos(this.phi)],this.rightVec=normalize([this.forwardVec[1],-this.forwardVec[0],0]),this.upVec=crossProduct(this.rightVec,this.forwardVec),this.lockedOnOrigin&&(this.cameraPos=scaleVector(-this.distanceFromOrigin,this.forwardVec)),this.speedFactor=Math.min(this.distanceEstimator(this.cameraPos[0],this.cameraPos[1],this.cameraPos[2]),.5)/4,this.forwardVec[0]*=this.speedFactor/1.5,this.forwardVec[1]*=this.speedFactor/1.5,this.forwardVec[2]*=this.speedFactor/1.5,this.rightVec[0]*=this.speedFactor/this.fovFactor,this.rightVec[1]*=this.speedFactor/this.fovFactor,this.upVec[0]*=this.speedFactor/this.fovFactor,this.upVec[1]*=this.speedFactor/this.fovFactor,this.upVec[2]*=this.speedFactor/this.fovFactor,this.imagePlaneCenterPos=[this.cameraPos[0]+this.forwardVec[0]*this.focalLengthFactor,this.cameraPos[1]+this.forwardVec[1]*this.focalLengthFactor,this.cameraPos[2]+this.forwardVec[2]*this.focalLengthFactor],this.setUniforms({cameraPos:this.cameraPos,imagePlaneCenterPos:this.imagePlaneCenterPos,rightVec:this.rightVec,upVec:this.upVec}),this.needNewFrame=!1)}distanceEstimator(){throw new Error("Distance estimator not implemented!")}prepareFrame(timeElapsed){this.touchDelay=Math.max(0,this.touchDelay-timeElapsed),this.moveUpdate(timeElapsed)}drawFrame(){this.theta=this.lockedOnOrigin?this.wilson.worldCenterX:2*Math.PI-this.wilson.worldCenterX,this.phi=this.lockedOnOrigin?this.wilson.worldCenterY:Math.PI-this.wilson.worldCenterY,this.calculateVectors(),this.useAntialiasing&&this.wilson.useFramebuffer("0"),this.wilson.drawFrame(),this.useAntialiasing&&(this.wilson.useShader("edgeDetect"),this.wilson.useTexture("0"),this.wilson.useFramebuffer("1"),this.wilson.drawFrame(),this.wilson.useShader("antialias"),this.wilson.useTexture("1"),this.wilson.useFramebuffer(null),this.wilson.drawFrame(),this.wilson.useShader("draw"))}async downloadFrame(filename){this.drawFrame(),this.wilson.downloadFrame(filename,!1)}async makeMosaic({size=16,returnPixels=!1,useForDepthBuffer=!1}){return new Promise(outerResolve=>{this.setUniforms({uvScale:1/size,resolution:this.resolution*size});const i=[];for(let e=0;e<size;e++)i.push((1+2*e)/size-1);const a=[],r=new Array(size);for(let t=0;t<size;t++){a.push([]),r[t]=new Array(size);for(let e=0;e<size;e++)a[t].push(document.createElement("canvas")),a[t][e].width=this.resolution,a[t][e].height=this.resolution}const s=async()=>{if(useForDepthBuffer){var s=new Float32Array(this.resolution*this.resolution*size*size);for(let i=0;i<size;i++)for(let t=0;t<this.resolution;t++)for(let e=0;e<size;e++)s.set(r[e][i].subarray(t*this.resolution,(t+1)*this.resolution),i*this.resolution*this.resolution*size+t*this.resolution*size+e*this.resolution);outerResolve(s)}else{var e=document.createElement("canvas"),i=(e.width=this.resolution*size,e.height=this.resolution*size,e.getContext("2d",{colorSpace:"display-p3"}));for(let t=0;t<size;t++)for(let e=0;e<size;e++)i.drawImage(a[t][e],t*this.resolution,e*this.resolution);returnPixels||(i.translate(0,this.resolution*size),i.scale(1,-1)),i.drawImage(e,0,0);var o=returnPixels?i.getImageData(0,0,this.resolution*size,this.resolution*size).data:void 0;returnPixels||e.toBlob(blob=>{var e=document.createElement("a");e.href=URL.createObjectURL(blob),e.download="mosaic.png",e.click()}),await sleep(100),this.setUniforms({uvScale:1,uvCenter:[0,0],resolution:this.resolution}),this.needNewFrame=!0,outerResolve(o)}};let o=0,l=0;const n=async()=>{var e=a[o][l].getContext("2d",{colorSpace:"display-p3"}),t=(this.setUniforms({uvCenter:[i[o],i[l]]}),this.drawFrame(),useForDepthBuffer&&(r[o][l]=new Float32Array(this.wilson.readPixels().buffer)),new ImageData(new Uint8ClampedArray(this.wilson.readPixels()),this.resolution,this.resolution));e.putImageData(t,0,0),await sleep(500),++l===size&&(l=0,o++),o!==size?requestAnimationFrame(n):s()};requestAnimationFrame(n)})}async downloadBokehFrame(){var e=this.useAntialiasing;await loadGlsl(),this.reloadShader({useAntialiasing:!0,useForDepthBuffer:!1});const t=await this.makeMosaic({size:10,returnPixels:!0});this.reloadShader({useAntialiasing:!1,useForDepthBuffer:!0});var s=await this.makeMosaic({size:10,useForDepthBuffer:!0});const o=10*this.resolution;var a=o/250,r=this.createHiddenCanvas(),l={canvasWidth:o},l=new WilsonCPU(r,l),n=(l.ctx.fillStyle="rgb(0, 0, 0)",l.ctx.fillRect(0,0,o,o),Array.from(s).map((depth,i)=>{return[depth,i%o,o-Math.floor(i/o)-1,t[4*i],t[4*i+1],t[4*i+2]]}).sort((a,b)=>b[0]-a[0])),c=n[n.length-1][0];let h=0;for(let x=0;x<n.length;x++)if(n[x][0]<this.clipDistance){h=x;break}var m=n[h=Math.floor(h+.1*(n.length-h))][0],u=new Array(o*o*4).fill(0);for(const F of n){var d=F[0];if(d!==this.clipDistance){var p,g=F[1],f=F[2],v=Math.min(Math.max((d-c)/(m-c),0),1)*(a-.5)+.5;let t=0;for(let i=Math.floor(-v);i<=Math.ceil(v);i++)for(let e=Math.floor(-v);e<=Math.ceil(v);e++)e*e+i*i<=v*v&&(p=Math.min(v-Math.sqrt(e*e+i*i),1),t+=p);var w,S,y=1/t;for(let s=Math.floor(-v);s<=Math.ceil(v);s++)for(let e=Math.floor(-v);e<=Math.ceil(v);e++)e*e+s*s<=v*v&&(w=f+s,S=g+e,w<0||w>=o||S<0||S>=o||(w=w*o+S,S=y*Math.min(v-Math.sqrt(e*e+s*s),1),u[4*w]+=F[3]*S,u[4*w+1]+=F[4]*S,u[4*w+2]+=F[5]*S))}}var P=new Uint8ClampedArray(u);for(let G=0;G<o*o;G++)P[4*G+3]=255;l.drawFrame(P),l.downloadFrame("bokeh-render.png"),this.reloadShader({useAntialiasing:e,useForDepthBuffer:!1}),this.drawFrame(),r.remove()}touchDelay=0;numTouches=0;onTouchStart({event}){this.numTouches<=1&&2===event.touches.length?(this.numTouches=2,this.touchDelay=100):this.numTouches<=2&&3===event.touches.length?this.numTouches=3:this.numTouches=0}onTouchEnd({event}){event.touches.length<2&&(this.numTouches=0)}moveUpdate(timeElapsed){0===this.touchDelay&&(2===this.numTouches?this.moveVelocity[0]=1:3===this.numTouches&&(this.moveVelocity[0]=-1)),this.keysPressed.w||2===this.numTouches?this.moveVelocity[0]=1:!this.keysPressed.s&&3!==this.numTouches||(this.moveVelocity[0]=-1),this.keysPressed.d?this.moveVelocity[1]=1:this.keysPressed.a&&(this.moveVelocity[1]=-1),this.keysPressed[" "]?this.moveVelocity[2]=1:this.keysPressed.shift&&(this.moveVelocity[2]=-1);var e,t,i=(this.keysPressed.control?.1:1)*this.movingSpeed;this.lockedOnOrigin||0===this.moveVelocity[0]&&0===this.moveVelocity[1]&&0===this.moveVelocity[2]||(t=void 0!==this.lockZ?scaleVector(magnitude(this.forwardVec),normalize([this.forwardVec[0],this.forwardVec[1],0])):this.forwardVec,e=void 0!==this.lockZ?scaleVector(magnitude(this.rightVec),normalize([this.rightVec[0],this.rightVec[1],0])):this.rightVec,t=[this.moveVelocity[0]*t[0]+this.moveVelocity[1]*e[0],this.moveVelocity[0]*t[1]+this.moveVelocity[1]*e[1],this.moveVelocity[0]*t[2]+this.moveVelocity[1]*e[2]+this.moveVelocity[2]*this.speedFactor/1.5],this.cameraPos[0]+=i*t[0]*(timeElapsed/6.944),this.cameraPos[1]+=i*t[1]*(timeElapsed/6.944),this.cameraPos[2]=this.lockZ??this.cameraPos[2]+i*t[2]*(timeElapsed/6.944),this.needNewFrame=!0);for(let s=0;s<3;s++)this.moveVelocity[s]*=this.moveFriction**(timeElapsed/6.944),Math.abs(this.moveVelocity[s])<this.moveStopThreshhold&&(this.moveVelocity[s]=0)}onResizeCanvas(){this.useFor3DPrinting||(this.resolution=Math.sqrt(this.wilson.canvasWidth*this.wilson.canvasHeight),this.wilson.resizeWorld({minY:.001-this.wilson.worldHeight/2,maxY:Math.PI-.001+this.wilson.worldHeight/2}),this.setUniforms({aspectRatio:[this.wilson.worldWidth/this.worldSize,this.wilson.worldHeight/this.worldSize],resolution:this.resolution}),this.useAntialiasing&&this.createTextures(),this.needNewFrame=!0)}animateUniform({name,value,duration=1e3}){const e={t:this.uniforms[name]};return anime({targets:e,t:value,duration:duration,easing:"easeInOutQuart",update:()=>{this.setUniforms({[name]:e.t}),this.needNewFrame=!0}}).finished}loopUniform({name,startValue,endValue,duration=2e3}){const e={t:0};return anime({targets:e,t:1,duration:duration,easing:"easeInOutQuad",loop:!0,direction:"alternate",update:()=>{this.setUniforms({[name]:startValue+(endValue-startValue)*e.t}),this.needNewFrame=!0}})}async setLockedOnOrigin(value){if(value&&!this.lockedOnOrigin){var t=magnitude(this.cameraPos);const s=normalize(this.cameraPos);var i=Math.acos(this.cameraPos[2]/t);let e=Math.PI-Math.atan2(this.cameraPos[1],this.cameraPos[0]);e>Math.PI&&(e-=2*Math.PI),e<-Math.PI&&(e+=2*Math.PI);const o={r:t,theta:2*Math.PI-this.theta,phi:Math.PI-this.phi};o.theta>Math.PI&&(o.theta-=2*Math.PI),o.theta<-Math.PI&&(o.theta+=2*Math.PI),await anime({targets:o,theta:e,phi:i,r:this.distanceFromOrigin,duration:500,easing:"easeOutCubic",update:()=>{this.wilson.resizeWorld({centerX:o.theta,centerY:o.phi}),this.cameraPos=scaleVector(o.r,s),this.needNewFrame=!0}}).finished}this.lockedOnOrigin=value,this.worldSize=this.lockedOnOrigin?2.5:1.5,this.wilson.resizeWorld({width:this.worldSize,height:this.worldSize,centerX:this.lockedOnOrigin?this.theta:2*Math.PI-this.theta,centerY:this.lockedOnOrigin?this.phi:Math.PI-this.phi,minY:.001-this.worldSize/2,maxY:Math.PI-.001+this.worldSize/2})}}function magnitude(vec){return Math.sqrt(vec[0]*vec[0]+vec[1]*vec[1]+vec[2]*vec[2])}function addVectors(vec1,vec2){return[vec1[0]+vec2[0],vec1[1]+vec2[1],vec1[2]+vec2[2]]}function scaleVector(c,vec){return[c*vec[0],c*vec[1],c*vec[2]]}function dotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]+vec1[2]*vec2[2]}function dotProduct4(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]+vec1[2]*vec2[2]+vec1[3]*vec2[3]}function crossProduct(vec1,vec2){return[vec1[1]*vec2[2]-vec1[2]*vec2[1],vec1[2]*vec2[0]-vec1[0]*vec2[2],vec1[0]*vec2[1]-vec1[1]*vec2[0]]}function matMul(mat1,mat2){return[[mat1[0][0]*mat2[0][0]+mat1[0][1]*mat2[1][0]+mat1[0][2]*mat2[2][0],mat1[0][0]*mat2[0][1]+mat1[0][1]*mat2[1][1]+mat1[0][2]*mat2[2][1],mat1[0][0]*mat2[0][2]+mat1[0][1]*mat2[1][2]+mat1[0][2]*mat2[2][2]],[mat1[1][0]*mat2[0][0]+mat1[1][1]*mat2[1][0]+mat1[1][2]*mat2[2][0],mat1[1][0]*mat2[0][1]+mat1[1][1]*mat2[1][1]+mat1[1][2]*mat2[2][1],mat1[1][0]*mat2[0][2]+mat1[1][1]*mat2[1][2]+mat1[1][2]*mat2[2][2]],[mat1[2][0]*mat2[0][0]+mat1[2][1]*mat2[1][0]+mat1[2][2]*mat2[2][0],mat1[2][0]*mat2[0][1]+mat1[2][1]*mat2[1][1]+mat1[2][2]*mat2[2][1],mat1[2][0]*mat2[0][2]+mat1[2][1]*mat2[1][2]+mat1[2][2]*mat2[2][2]]]}function qmul(x1,y1,z1,w1,x2,y2,z2,w2){return[x1*x2-y1*y2-z1*z1-w1*w2,x1*y2+y1*x2+z1*w2-w1*z2,x1*z2-y1*w2+z1*x2+w1*y2,x1*w2+y1*z2-z1*y2+w1*x2]}function normalize(vec){var e=magnitude(vec);return[vec[0]/e,vec[1]/e,vec[2]/e]}function getRotationMatrix(thetaX,thetaY,thetaZ){var e=Math.cos(thetaX),t=Math.sin(thetaX),i=Math.cos(thetaY),s=Math.sin(thetaY),o=Math.cos(thetaZ),a=Math.sin(thetaZ),t=[[1,0,0],[0,e,-t],[0,t,e]];return matMul(matMul([[o,-a,0],[a,o,0],[0,0,1]],[[i,0,-s],[0,1,0],[s,0,i]]),t)}function mat3TimesVector(mat,vec){return[mat[0][0]*vec[0]+mat[0][1]*vec[1]+mat[0][2]*vec[2],mat[1][0]*vec[0]+mat[1][1]*vec[1]+mat[1][2]*vec[2],mat[2][0]*vec[0]+mat[2][1]*vec[1]+mat[2][2]*vec[2]]}export{edgeDetectShader,RaymarchApplet,magnitude,addVectors,scaleVector,dotProduct,dotProduct4,crossProduct,matMul,qmul,normalize,getRotationMatrix,mat3TimesVector};