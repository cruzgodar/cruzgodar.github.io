import{showCard}from"../src/cards.min.js";import{addHoverEventWithScale}from"../src/hoverEvents.min.js";import{$,addTemporaryListener,pageElement,pageUrl}from"../src/main.min.js";const glslFunctions={cadd:["vec2","vec2","vec2"],csub:["vec2","vec2","vec2"],cmul:["vec2","vec2","vec2"],cdiv:["vec2","vec2","vec2"],cexp:["vec2","vec2"],csin:["vec2","vec2"],ccos:["vec2","vec2"]};class Applet{canvas;wilson;wilsonForFullscreen;allowFullscreenWithKeyboard=!0;fpsDisplayCtx;workers=[];timeoutIds=[];keysPressed={};numTouches=0;needNewFrame=!1;aspectRatio=1;state={};defaultState={};constructor(canvas){if(this.canvas=canvas,(this.pan.parent=this).zoom.parent=this,window.DEBUG&&setTimeout(()=>this.addFpsDisplay(),500),$("#applet-controls-card")&&!this.addHelpButton()){const t=setInterval(()=>{this.addHelpButton()&&clearInterval(t)},50)}Applet.current.push(this)}destroy(){this.animationPaused=!0,this.workers.forEach(worker=>{worker?.terminate&&worker?.terminate()}),this.timeoutIds.forEach(timeoutId=>{null!=timeoutId&&clearTimeout(timeoutId)}),this.hiddenCanvasContainer&&this.hiddenCanvasContainer.remove();var t=new URLSearchParams(window.location.search);for(const i in this.state)t.delete(i);var e=t.toString(),e=(window.history.replaceState({url:pageUrl},document.title,pageUrl.replace(/\/home\//,"/")+(e?"?"+e:"")),["a","e","i","o","u","y"].includes(this.constructor.name[0].toLowerCase())?"n":"");console.log(`Destroyed a${e} ${this.constructor.name} applet`)}initState(){var t=new URLSearchParams(window.location.search);for(const e in this.defaultState)this.state[e]=t.get(e)}setState(key,value){this.state[key]=value;var t=new URLSearchParams(window.location.search),t=(t.set(key,value),t.toString());window.history.replaceState({url:pageUrl},"",pageUrl.replace(/\/home\//,"/")+(t?"?"+t:""))}runWhenOnscreen(data){let i=!1;var t=()=>{var t,e;i||(e=(t=this.canvas.getBoundingClientRect()).top,-t.height<=e&&e<window.innerHeight&&(i=!0,setTimeout(()=>this.run(data),250)))};addTemporaryListener({object:window,event:"scroll",callback:t}),t()}addFpsDisplay(){var t=document.createElement("canvas");t.classList.add("fps-canvas"),t.setAttribute("width","100"),t.setAttribute("height","100"),this.canvas.parentNode.insertBefore(t,this.canvas.nextElementSibling),this.fpsDisplayCtx=t.getContext("2d"),requestAnimationFrame(this.updateFpsDisplay.bind(this))}lastFpsDisplayTimestamp;fpsTimestampHistory=new Array(100);updateFpsDisplay(timestamp){var e=timestamp-this.lastFpsDisplayTimestamp;if(e&&this.lastFpsDisplayTimestamp){this.lastFpsDisplayTimestamp=timestamp,this.fpsTimestampHistory.push(e),this.fpsTimestampHistory.shift(),this.fpsDisplayCtx.clearRect(0,0,100,100);for(let t=0;t<100;t++){const a=Math.min(this.fpsTimestampHistory[t],100);var i=hsvToRgb(a<8.333?5/9-a/8.333*(5/9-1/3):a<16.667?1/3-(a-8.333)/(16.667-8.333)*(1/3-7/45):a<33.333?7/45-(a-16.667)/(33.333-16.667)*(7/45-1/12):1/12-(a-33.333)/66.667*1/12,1,1);this.fpsDisplayCtx.fillStyle=`rgb(${i[0]}, ${i[1]}, ${i[2]})`,this.fpsDisplayCtx.fillRect(t,100-a,1,a)}this.fpsDisplayCtx.fillStyle="rgb(127, 127, 127)",this.fpsDisplayCtx.fillRect(0,91.667,100,2),this.fpsDisplayCtx.fillRect(0,83.333,100,2),this.fpsDisplayCtx.fillRect(0,66.667,100,2)}else this.lastFpsDisplayTimestamp=timestamp;requestAnimationFrame(this.updateFpsDisplay.bind(this))}hiddenCanvasContainer;createHiddenCanvas(){var t=document.createElement("canvas");return t.classList.add("hidden-canvas"),this.hiddenCanvasContainer||(this.hiddenCanvasContainer=document.createElement("div"),this.hiddenCanvasContainer.style.display="none",pageElement.appendChild(this.hiddenCanvasContainer)),this.hiddenCanvasContainer.appendChild(t),t}addHelpButton(){if(!this?.wilson?.outputCanvasContainer)return!1;const t=document.createElement("div");return t.classList.add("wilson-help-button"),t.innerHTML=`
		...
		)`:(console.error("Invalid matrix shape!"),"")}const tempShader=`
...
`;export{Applet,getMinGlslString,getMaxGlslString,getColorGlslString,getFloatGlsl,getVectorGlsl,getMatrixGlsl,tempShader};