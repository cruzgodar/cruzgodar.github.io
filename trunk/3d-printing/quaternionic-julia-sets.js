!function()
{
	...
	let frag_shader_source = `
		...
		float distance_estimator(vec3 pos)
		{
			...
			for (int iteration = 0; iteration < 100; iteration++)
			{
				...
				z += mix(vec4(pos, 0.0), vec4(c, 0.0), julia_proportion);
			}
		...
		vec3 compute_shading(vec3 pos, int iteration)
		{
			...
			vec3 color = vec3(1.0, 1.0, 1.0) * light_intensity * max((1.0 - float(iteration) / float(max_marches)), 0.0);
		...
		vec3 raymarch(vec3 start_pos)
		{
			...
			vec3 ray_direction_vec = normalize(vec3(0.0, 0.0, -1.0)) * .9 / step_factor;
		...
		void main(void)
		{
			vec3 pos = vec3(uv * 1.5, k_slice);
			
			float epsilon = .003;
			
			if (distance_estimator(pos) < epsilon)
			{
	...
	let max_marches = 1000;
	
	let image_plane_center_pos = [];
	
	let forward_vec = [0, 0, -1];
	let right_vec = [2, 0, 0];
	let up_vec = [0, 2, 0];
	
	let camera_pos = [0, 0, 3];
	...
	let c_w_input_element = Page.element.querySelector("#c-w-input");
	...
	wilson.gl.uniform1f(wilson.uniforms["k_slice"], -2);
	...
	setTimeout(() => window.requestAnimationFrame(draw_frame), 1000);
	...
	function draw_frame(timestamp)
	{
		...
		wilson.download_frame(`${frame}.png`);
		
		frame++;
		k_slice = -1.5 + frame * 3 / 1000;
		wilson.gl.uniform1f(wilson.uniforms["k_slice"], k_slice);
		
		if (frame < 1000)
		{
			setTimeout(() => window.requestAnimationFrame(draw_frame), 500);
		}	
	}
	...
	function switch_bulb()
	{
		...
		if (julia_proportion === 0)
		{
			...
			setTimeout(() =>
			{
				...
				Page.Animate.change_opacity(randomize_c_button_element, 1, Site.opacity_animation_time);
			}, Site.opacity_animation_time);
		}
		...
		else
		{
			...
			Page.Animate.change_opacity(randomize_c_button_element, 0, Site.opacity_animation_time);
		}
