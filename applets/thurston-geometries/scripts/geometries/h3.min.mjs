import{BaseGeometry,getMinGlslString}from"./base.min.mjs";class H3Geometry extends BaseGeometry{geodesicGlsl="vec4 pos = cosh(t) * startPos + sinh(t) * rayDirectionVec;";dotProductGlsl="return v.x * w.x + v.y * w.y + v.z * w.z - v.w * w.w;";normalizeGlsl=`float magnitude = sqrt(abs(geometryDot(dir, dir)));
	...
	`;correctVectors(){var t=this.dotProduct(this.normalVec,this.upVec),e=this.dotProduct(this.normalVec,this.rightVec),o=this.dotProduct(this.normalVec,this.forwardVec);for(let s=0;s<4;s++)this.upVec[s]+=t*this.normalVec[s],this.rightVec[s]+=e*this.normalVec[s],this.forwardVec[s]+=o*this.normalVec[s];this.upVec=this.normalize(this.upVec),this.rightVec=this.normalize(this.rightVec),this.forwardVec=this.normalize(this.forwardVec)}getMovingSpeed(){return.25}cameraPos=[0,0,0,1];normalVec=[0,0,0,-1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0]}export{H3Spheres};