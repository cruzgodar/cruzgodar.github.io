import{BaseGeometry,getMinGlslString}from"./base.min.mjs";class H3Geometry extends BaseGeometry{geodesicGlsl="vec4 pos = cosh(t) * cameraPos + sinh(t) * rayDirectionVec;";fogGlsl=`return mix(
		color,
		fogColor,
	...
	}`;dotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]+vec1[2]*vec2[2]-vec1[3]*vec2[3]}normalize(vec){var e=Math.sqrt(Math.abs(this.dotProduct(vec,vec)));return[vec[0]/e,vec[1]/e,vec[2]/e,vec[3]/e]}updateCameraPos(cameraPos,tangentVec,dt){var e=new Array(4);for(let o=0;o<4;o++)e[o]=Math.cosh(dt)*cameraPos[o]+Math.sinh(dt)*tangentVec[o];var t=Math.sqrt(-e[0]*e[0]-e[1]*e[1]-e[2]*e[2]+e[3]*e[3]);return e[0]/=t,e[1]/=t,e[2]/=t,e[3]/=t,e}getNormalVec(cameraPos){return this.normalize([cameraPos[0],cameraPos[1],cameraPos[2],-cameraPos[3]])}getGammaPrime(_pos,dir){return[...dir]}getGammaDoublePrime(pos){return[...pos]}getGammaTriplePrime(_pos,dir){return[...dir]}gammaTriplePrimeIsLinearlyIndependent=!1}class H3Spheres extends H3Geometry{static distances=`
