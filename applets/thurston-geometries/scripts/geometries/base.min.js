class BaseGeometry{geodesicGlsl=`
	...
	`;updateTGlsl=`
		...
		t += lastTIncrease;
	`;maxMarches="200";maxT="50.0";ambientOcclusionDenominator="100.0";stepFactor="0.99";dotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]+vec1[2]*vec2[2]+vec1[3]*vec2[3]}normalize(vec){var t=Math.sqrt(this.dotProduct(vec,vec));return[vec[0]/t,vec[1]/t,vec[2]/t,vec[3]/t]}followGeodesic(pos,dir,t){return[pos[0]+t*dir[0],pos[1]+t*dir[1],pos[2]+t*dir[2],pos[3]+t*dir[3]]}teleportCamera(){}getNormalVec(){return[0,0,0,1]}correctPosition(pos){return pos}correctVectors(){var t=this.dotProduct(this.normalVec,this.upVec),r=this.dotProduct(this.normalVec,this.rightVec),e=this.dotProduct(this.normalVec,this.forwardVec);for(let o=0;o<4;o++)this.upVec[o]-=t*this.normalVec[o],this.rightVec[o]-=r*this.normalVec[o],this.forwardVec[o]-=e*this.normalVec[o];this.upVec=this.normalize(this.upVec),this.rightVec=this.normalize(this.rightVec),this.forwardVec=this.normalize(this.forwardVec)}distanceEstimatorGlsl;getColorGlsl;lightGlsl;functionGlsl;raymarchSetupGlsl;finalTeleportationGlsl;usesFiberComponent=!1;cameraPos;normalVec;upVec;rightVec;forwardVec;lockedOnOrigin=!1;render1D=!1;movingSpeed=1;aspectRatio;fov;static IGNORE_MODIFIER=0;static DISALLOW_MODIFIER=1;static REQUIRE_MODIFIER=2;controlMode=BaseGeometry.IGNORE_MODIFIER;uniformGlsl;uniformNames;uniformData={};updateUniforms(){}initUI(){}uiElementsUsed="";sliderValues={wallThickness:0,fiberThickness:0,gluingAngle:0}}function getMinGlslString(varName,numVars,functionName="min"){var t=Math.ceil(Math.log2(numVars));let r=new Array(numVars);for(let o=0;o<numVars;o++)r[o]=""+varName+(o+1);for(let a=0;a<t;a++){var e=new Array(Math.ceil(r.length/2));for(let t=0;t<r.length;t+=2)e[t/2]=`${functionName}(${r[t]}, ${r[t+1]})`;r.length%2==1&&(e[e.length-1]=r[r.length-1]),r=e}return r[0]}function getMaxGlslString(varName,numVars){return getMinGlslString(varName,numVars,"max")}function getColorGlslString(varName,minVarName,colors){let t="";for(let r=0;r<colors.length;r++)t+=`if (${minVarName} == ${varName}${r+1}) { return vec3(${colors[r][0]/255}, ${colors[r][1]/255}, ${colors[r][2]/255}); }
