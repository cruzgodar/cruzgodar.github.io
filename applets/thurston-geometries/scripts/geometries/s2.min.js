...
`;class E3S2Demo extends E3Geometry{numRays=15;rayDirs=Array(this.numRays).fill([0,0,0,0]);testVecs=Array(this.numRays).fill([0,0,0,0]);rayLengths=Array(this.numRays).fill(0);rayColors=Array(this.numRays).fill([0,0,0]);distanceEstimatorGlsl=`
	...
	`;uniformNames=["cameraDotPos",...this.rayLengths.map((_,index)=>"rayDir"+(index+1)),...this.rayLengths.map((_,index)=>"testVec"+(index+1)),...this.rayLengths.map((_,index)=>"rayLength"+(index+1)),...this.rayLengths.map((_,index)=>"rayColor"+(index+1))];updateUniforms(gl,uniformList,programIndex){gl.uniform4fv(uniformList.cameraDotPos[programIndex],this.cameraDotPos);for(let i=0;i<this.numRays;i++)gl.uniform4fv(uniformList["rayDir"+(i+1)][programIndex],this.rayDirs[i]),gl.uniform4fv(uniformList["testVec"+(i+1)][programIndex],this.testVecs[i]),gl.uniform1f(uniformList["rayLength"+(i+1)][programIndex],this.rayLengths[i]),gl.uniform3fv(uniformList["rayColor"+(i+1)][programIndex],this.rayColors[i])}}const s2xeDemoDistances=`
	float radius = .35;
