import{BaseGeometry,getMinGlslString}from"./base.min.js";import{E3Geometry}from"./e3.min.js";import{S2xEGeometry}from"./s2xe.min.js";class E3S2Demo extends E3Geometry{static distances=`
		// The sphere itself.
	...
	`;correctPosition(pos){var e=Math.sqrt(pos[0]*pos[0]+pos[1]*pos[1]+pos[2]*pos[2])/2.5;return[pos[0]/e,pos[1]/e,pos[2]/e,1]}correctVectors(){this.forwardVec=this.normalize([-this.cameraPos[0],-this.cameraPos[1],0,0]);var e=this.dotProduct(this.forwardVec,this.rightVec);for(let t=0;t<4;t++)this.rightVec[t]-=e*this.forwardVec[t];this.rightVec=this.normalize(this.rightVec)}cameraPos=[-2.5,0,0,1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];movingSpeed=5;lockedOnOrigin=!0;fov=Math.tan(30*Math.PI/180);controlMode=BaseGeometry.REQUIRE_MODIFIER;cameraDotPos=[0,0,1];uniformGlsl=`
		uniform vec3 cameraDotPos;
	`;uniformNames=["cameraDotPos"];updateUniforms(gl,uniformList){gl.uniform3fv(uniformList.cameraDotPos,this.cameraDotPos)}}class S2xES2Demo extends S2xEGeometry{static distances=`
