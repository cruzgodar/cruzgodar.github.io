import{getMinGlslString}from"../../../../scripts/applets/applet.min.js";import{E3Geometry}from"./e3.min.js";import{S2xEGeometry}from"./s2xe.min.js";function getSphereBandGlsl(index){return`
	...
	`;render1D=!0;cameraPos=[0,0,1,0];normalVec=[0,0,1,0];upVec=[0,0,0,1];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];aspectRatio=7;ignoreAspectRatio=!0;distanceEstimator(pos){var t=.34,e=Math.acos(pos[0])-t,a=Math.acos(-pos[0])-t,i=Math.acos(pos[1])-t,s=Math.acos(-pos[1])-t,t=Math.acos(-pos[2])-t;return Math.min(Math.min(Math.min(e,a),Math.min(i,s)),t)}getColor(pos){var t=.35,e=Math.acos(pos[0])-t,a=Math.acos(-pos[0])-t,i=Math.acos(pos[1])-t,s=Math.acos(-pos[1])-t,t=Math.acos(-pos[2])-t,t=Math.min(Math.min(Math.min(e,a),Math.min(i,s)),t);return 1e-5<t?[0,0,0]:t===e?[1,0,0]:t===a?[0,0,1]:t===i?[0,1,0]:t===s?[1,1,0]:[.5,0,1]}getRayData(rayDirs){var a=new Array(rayDirs.length),i=new Array(rayDirs.length);for(let o=0;o<rayDirs.length;o++){let t=0;var s=[...this.cameraPos];let e=[...s];for(var r;r=this.distanceEstimator(e),t+=.99*r,e=[Math.cos(t)*s[0]+Math.sin(t)*rayDirs[o][0],Math.cos(t)*s[1]+Math.sin(t)*rayDirs[o][1],Math.cos(t)*s[2]+Math.sin(t)*rayDirs[o][2],0],1e-5<r&&t<2*Math.PI;);a[o]=t,i[o]=this.getColor(e)}return[a,i]}}export{E3S2Demo,S2xES2Demo};