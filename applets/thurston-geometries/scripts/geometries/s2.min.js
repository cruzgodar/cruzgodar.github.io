import{getMinGlslString}from"./base.min.js";import{E3Geometry}from"./e3.min.js";import{S2xEGeometry}from"./s2xe.min.js";function getSphereBandGlsl(index){return`
	...
	`;render1D=!0;cameraPos=[0,0,1,0];normalVec=[0,0,1,0];upVec=[0,0,0,1];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];aspectRatio=7;distanceEstimator(pos){var t=Math.acos(pos[0])-.49,e=Math.acos(-pos[0])-.49,a=Math.acos(pos[1])-.49,i=Math.acos(-pos[1])-.49,s=Math.acos(-pos[2])-.49;return Math.min(Math.min(Math.min(t,e),Math.min(a,i)),s)}getColor(pos){var t=Math.acos(pos[0])-.5,e=Math.acos(-pos[0])-.5,a=Math.acos(pos[1])-.5,i=Math.acos(-pos[1])-.5,s=Math.acos(-pos[2])-.5,s=Math.min(Math.min(Math.min(t,e),Math.min(a,i)),s);return 1e-5<s?[0,0,0]:s===t?[1,0,0]:s===e?[0,0,1]:s===a?[0,1,0]:s===i?[1,1,0]:[.5,0,1]}getRayData(rayDirs){var a=new Array(rayDirs.length),i=new Array(rayDirs.length);for(let r=0;r<rayDirs.length;r++){let t=0;var s=[...this.cameraPos];let e=[...s];for(var o;o=this.distanceEstimator(e),t+=.99*o,e=[Math.cos(t)*s[0]+Math.sin(t)*rayDirs[r][0],Math.cos(t)*s[1]+Math.sin(t)*rayDirs[r][1],Math.cos(t)*s[2]+Math.sin(t)*rayDirs[r][2],0],1e-5<o&&t<2*Math.PI;);a[r]=t,i[r]=this.getColor(e)}return[a,i]}}export{E3S2Demo,S2xES2Demo};