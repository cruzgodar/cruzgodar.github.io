import{ThurstonGeometry}from"../class.min.js";import{BaseGeometry}from"./base.min.js";class NilGeometry extends BaseGeometry{geodesicGlsl=`mat4 A = mat4(
	...
	return dir / magnitude;`;fogGlsl="return color;//mix(color, fogColor, 1.0 - exp(0.5 - totalT * 0.075));";functionGlsl=`float chi(float rho, float z, float phi)
	{
	...
	`;normalize(vec){var t=vec[2]-(this.cameraPos[0]*vec[1]-this.cameraPos[1]*vec[0])/2,t=Math.sqrt(vec[0]*vec[0]+vec[1]*vec[1]+t*t);return[vec[0]/t,vec[1]/t,vec[2]/t,vec[3]/t]}followGeodesic(pos,dir,t){var o=[[1,0,0,pos[0]],[0,1,0,pos[1]],[-pos[1]/2,pos[0]/2,1,pos[2]],[0,0,0,1]],e=[[1,0,0,-pos[0]],[0,1,0,-pos[1]],[pos[1]/2,-pos[0]/2,1,-pos[2]],[0,0,0,1]],e=ThurstonGeometry.mat4TimesVector(e,dir),s=Math.atan2(e[1],e[0]),a=Math.sqrt(e[0]*e[0]+e[1]*e[1]),e=e[2];return ThurstonGeometry.mat4TimesVector(o,Math.abs(e)<1e-5?[a*Math.cos(s)*t,a*Math.sin(s)*t,0,0]:[2*a/e*Math.sin(e*t/2)*Math.cos(e*t/2+s),2*a/e*Math.sin(e*t/2)*Math.sin(e*t/2+s),e*t+a*a/(2*e*e)*(e*t-Math.sin(e*t)),0])}getNormalVec(){return[0,0,0,1]}lightGlsl=`
		...
		lightIntensity = 2.0;
	`;correctVectors(){}}class NilSpheres extends NilGeometry{static distances=`
		float distance1 = 1.0;
		/*
	...
	`;getMovingSpeed(){return 1.5}cameraPos=[0,0,0,1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0]}export{NilSpheres};