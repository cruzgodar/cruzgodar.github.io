import{ThurstonGeometry}from"../class.min.js";import{BaseGeometry}from"./base.min.js";class SL2RGeometry extends BaseGeometry{raymarchSetupGlsl=`
	...
	`;correctPosition(pos){var t=Math.sqrt(pos[0]*pos[0]+pos[1]*pos[1]-pos[2]*pos[2]-pos[3]*pos[3]);return[pos[0]/t,pos[1]/t,pos[2]/t,pos[3]/t]}getNormalVec(){return[0,0,1,0]}dotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]-vec1[2]*vec2[2]+vec1[3]*vec2[3]}normalize(vec){var t=Math.sqrt(Math.abs(this.dotProduct(vec,vec)));return[vec[0]/t,vec[1]/t,vec[2]/t,vec[3]/t]}followGeodesic(pos,dir,t){var e=Math.atan(dir[1],dir[0]),o=Math.sqrt(dir[0]*dir[0]+dir[1]*dir[1]),a=dir[3],r=Math.sqrt(Math.abs(a*a-o*o));let s,i;i=Math.abs(a)===o?(s=[1,-.35355339*t,.35355339*t,0],2*a*t+2*Math.atan(-.35355339*t)):Math.abs(a)>o?(n=r*t*.5,h=Math.sin(n),s=[Math.cos(n),-a/r*h,o/r*h,0],2*a*t+2*Math.atan(-a/r*Math.tan(n))-Math.sign(a)*Math.floor(.5*r*t/Math.PI+.5)*2*Math.PI):(h=r*t*.5,n=Math.sinh(h),s=[Math.cosh(h),-a/r*n,o/r*n,0],2*a*t+2*Math.atan(-a/r*Math.tanh(h)));var o=Math.sin(a*t),n=Math.cos(a*t),r=(s=[s[0]*n-s[1]*o,s[0]*o+s[1]*n,s[2]*n,-s[2]*o],Math.sin(e)),h=Math.cos(e);return s=[s[0],s[1],h*s[2]-r*s[3],r*s[2]+h*s[3]],this.cameraFiber+=i,ThurstonGeometry.mat4TimesVector([[pos[0],-pos[1],pos[2],pos[3]],[pos[1],pos[0],pos[3],-pos[2]],[pos[2],pos[3],pos[0],-pos[1]],[pos[3],-pos[2],pos[1],pos[0]]],s)}correctVectors(){}}class SL2RSpheres extends SL2RGeometry{static distances=`
		float radius = .25;
