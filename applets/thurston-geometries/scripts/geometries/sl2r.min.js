...
`;class SL2RAxes extends SL2RGeometry{geodesicGlsl=`
	...
	`;uniformNames=["cameraFiber"];updateUniforms(gl,uniformList){gl.uniform1f(uniformList.cameraFiber,this.cameraFiber)}}const roomsDistances=`
	vec3 h2Element = getH2Element(pos);
...
`;class SL2RRooms extends SL2RGeometry{distanceEstimatorGlsl=`
	...
	`;uniformNames=["cameraFiber","wallThickness","baseColor"];updateUniforms(gl,uniformList){gl.uniform1f(uniformList.cameraFiber,this.cameraFiber),gl.uniform1f(uniformList.wallThickness,1.85-this.sliderValues.wallThickness),gl.uniform1f(uniformList.clipDistance,this.sliderValues.clipDistance),gl.uniform3fv(uniformList.baseColor,this.baseColor)}uiElementsUsed="#wall-thickness-slider";wallThicknessData=[.1,.05,.15]}const spheresDistances=`
	vec3 h2Element = getH2Element(pos);
...
`;class SL2RSpheres extends SL2RGeometry{geodesicGlsl=`
	...
	`;stepFactor="0.3";cameraPos=loopRoomColors?[1,0,0,0]:[1.000000555682267,0,.0010542129021898201,0];cameraFiber=0;normalVec=[0,0,-1,0];upVec=[0,0,0,1];rightVec=[-Math.sqrt(2)/2,Math.sqrt(2)/2,0,0];forwardVec=[Math.sqrt(2)/2,Math.sqrt(2)/2,0,0];uniformGlsl=`
		uniform float cameraFiber;
		uniform vec3 baseColor;
	`;uniformNames=["cameraFiber","baseColor"];updateUniforms(gl,uniformList){gl.uniform1f(uniformList.cameraFiber,this.cameraFiber),gl.uniform3fv(uniformList.baseColor,this.baseColor)}}export{SL2RAxes,SL2RRooms,SL2RSpheres};