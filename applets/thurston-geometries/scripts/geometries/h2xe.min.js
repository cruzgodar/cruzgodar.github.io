import{ThurstonGeometry}from"../class.min.js";import{BaseGeometry,getMatrixGlsl,getMinGlslString,getVectorGlsl}from"./base.min.js";import{$}from"/scripts/src/main.min.js";const teleportations=[[[1,0,1/Math.sqrt(3),0],[[2,0,Math.sqrt(3),0],[0,1,0,0],[Math.sqrt(3),0,2,0],[0,0,0,1]]],[[-1,0,1/Math.sqrt(3),0],[[2,0,-Math.sqrt(3),0],[0,1,0,0],[-Math.sqrt(3),0,2,0],[0,0,0,1]]],[[0,1,1/Math.sqrt(3),0],[[1,0,0,0],[0,2,Math.sqrt(3),0],[0,Math.sqrt(3),2,0],[0,0,0,1]]],[[0,-1,1/Math.sqrt(3),0],[[1,0,0,0],[0,2,-Math.sqrt(3),0],[0,-Math.sqrt(3),2,0],[0,0,0,1]]]];class H2xEGeometry extends BaseGeometry{geodesicGlsl=`
	...
	`;maxT="30.0";correctPosition(pos){var t=Math.sqrt(-pos[0]*pos[0]-pos[1]*pos[1]+pos[2]*pos[2]);return[pos[0]/t,pos[1]/t,pos[2]/t,pos[3]]}getNormalVec(cameraPos){return console.log(this.cameraPos,this.forwardVec,this.rightVec,this.upVec,this.normalVec),this.normalize([-cameraPos[0],-cameraPos[1],cameraPos[2],0])}dotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]-vec1[2]*vec2[2]+vec1[3]*vec2[3]}normalize(vec){var t=Math.sqrt(Math.abs(this.dotProduct(vec,vec)));return[vec[0]/t,vec[1]/t,vec[2]/t,vec[3]/t]}correctVectors(){function t(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]-vec1[2]*vec2[2]}var e=t(this.cameraPos,this.upVec),o=t(this.cameraPos,this.rightVec),s=t(this.cameraPos,this.forwardVec);for(let r=0;r<3;r++)this.upVec[r]+=e*this.cameraPos[r],this.rightVec[r]+=o*this.cameraPos[r],this.forwardVec[r]+=s*this.cameraPos[r];this.upVec=this.normalize(this.upVec),this.rightVec=this.normalize(this.rightVec),this.forwardVec=this.normalize(this.forwardVec)}baseColorIncreases=[[0,1,0],[0,-1,0],[0,0,1],[0,0,-1]];baseColor=[0,0,0];teleportCamera(rotatedForwardVec,recomputeRotation){for(let t=0;t<teleportations.length;t++)ThurstonGeometry.dotProduct(this.cameraPos,teleportations[t][0])<0&&(this.cameraPos=ThurstonGeometry.mat4TimesVector(teleportations[t][1],this.cameraPos),this.forwardVec=ThurstonGeometry.mat4TimesVector(teleportations[t][1],this.forwardVec),this.rightVec=ThurstonGeometry.mat4TimesVector(teleportations[t][1],this.rightVec),this.upVec=ThurstonGeometry.mat4TimesVector(teleportations[t][1],this.upVec),recomputeRotation(ThurstonGeometry.mat4TimesVector(teleportations[t][1],rotatedForwardVec)),this.baseColor[0]+=this.baseColorIncreases[t][0],this.baseColor[1]+=this.baseColorIncreases[t][1],this.baseColor[2]+=this.baseColorIncreases[t][2])}}class H2xEAxes extends H2xEGeometry{geodesicGlsl=`
		float h2Mag = sqrt(abs(
			rayDirectionVec.x * rayDirectionVec.x
			+ rayDirectionVec.y * rayDirectionVec.y
			- rayDirectionVec.z * rayDirectionVec.z
	...
	`;cameraPos=[0,0,1,0];normalVec=[0,0,-1,0];upVec=[0,0,0,1];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];movingSpeed=1.25;uniformGlsl=`
		uniform float wallThickness;
		uniform vec3 baseColor;
	`;uniformNames=["wallThickness","baseColor"];updateUniforms(gl,uniformList){var t=1.145-this.sliderValues.wallThickness/10;gl.uniform1f(uniformList.wallThickness,t),gl.uniform3fv(uniformList.baseColor,this.baseColor)}uiElementsUsed="#wall-thickness-slider";initUI(){var t=$("#wall-thickness-slider"),e=$("#wall-thickness-slider-value");t.min=-.55,t.max=1.55,t.value=1.55,e.textContent=1.55,this.sliderValues.wallThickness=1.55}}class H2xESpheres extends H2xEGeometry{static distances=`
		float spacing = 1.5;
	...
	`;cameraPos=[0,0,1,.75];normalVec=[0,0,-1,0];upVec=[0,0,0,1];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];movingSpeed=1.25;uniformGlsl=`
		uniform vec3 baseColor;
	`;uniformNames=["baseColor"];updateUniforms(gl,uniformList){gl.uniform3fv(uniformList.baseColor,this.baseColor)}}export{H2xEAxes,H2xERooms,H2xESpheres};