import{getMatrixGlsl,getMinGlslString,getVectorGlsl}from"../../../../scripts/applets/applet.min.js";import{dotProduct,mat4TimesVector}from"../class.min.js";import{BaseGeometry}from"./base.min.js";const teleportations=[[[1,0,1/Math.sqrt(3),0],[[2,0,Math.sqrt(3),0],[0,1,0,0],[Math.sqrt(3),0,2,0],[0,0,0,1]]],[[-1,0,1/Math.sqrt(3),0],[[2,0,-Math.sqrt(3),0],[0,1,0,0],[-Math.sqrt(3),0,2,0],[0,0,0,1]]],[[0,1,1/Math.sqrt(3),0],[[1,0,0,0],[0,2,Math.sqrt(3),0],[0,Math.sqrt(3),2,0],[0,0,0,1]]],[[0,-1,1/Math.sqrt(3),0],[[1,0,0,0],[0,2,-Math.sqrt(3),0],[0,-Math.sqrt(3),2,0],[0,0,0,1]]]];class H2xEGeometry extends BaseGeometry{geodesicGlsl=`
	...
	`;maxT="30.0";correctPosition(pos){var t=Math.sqrt(-pos[0]*pos[0]-pos[1]*pos[1]+pos[2]*pos[2]);return[pos[0]/t,pos[1]/t,pos[2]/t,pos[3]]}getNormalVec(cameraPos){return this.normalize([-cameraPos[0],-cameraPos[1],cameraPos[2],0])}dotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]-vec1[2]*vec2[2]+vec1[3]*vec2[3]}normalize(vec){var t=Math.sqrt(Math.abs(this.dotProduct(vec,vec)));return[vec[0]/t,vec[1]/t,vec[2]/t,vec[3]/t]}correctVectors(){function t(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]-vec1[2]*vec2[2]}var e=t(this.cameraPos,this.upVec),o=t(this.cameraPos,this.rightVec),s=t(this.cameraPos,this.forwardVec);for(let r=0;r<3;r++)this.upVec[r]+=e*this.cameraPos[r],this.rightVec[r]+=o*this.cameraPos[r],this.forwardVec[r]+=s*this.cameraPos[r];this.upVec=this.normalize(this.upVec),this.rightVec=this.normalize(this.rightVec),this.forwardVec=this.normalize(this.forwardVec)}baseColorIncreases=[[0,1,0],[0,-1,0],[0,0,1],[0,0,-1]];baseColor=[0,0,0];teleportCamera(rotatedForwardVec,recomputeRotation){for(let t=0;t<teleportations.length;t++)dotProduct(this.cameraPos,teleportations[t][0])<0&&(this.cameraPos=mat4TimesVector(teleportations[t][1],this.cameraPos),this.forwardVec=mat4TimesVector(teleportations[t][1],this.forwardVec),this.rightVec=mat4TimesVector(teleportations[t][1],this.rightVec),this.upVec=mat4TimesVector(teleportations[t][1],this.upVec),recomputeRotation(mat4TimesVector(teleportations[t][1],rotatedForwardVec)),this.baseColor[0]+=this.baseColorIncreases[t][0],this.baseColor[1]+=this.baseColorIncreases[t][1],this.baseColor[2]+=this.baseColorIncreases[t][2])}}class H2xEAxes extends H2xEGeometry{geodesicGlsl=`
		float h2Mag = sqrt(abs(
			rayDirectionVec.x * rayDirectionVec.x
			+ rayDirectionVec.y * rayDirectionVec.y
			- rayDirectionVec.z * rayDirectionVec.z
	...
	`;uniformNames=["sceneTransition","wallThickness","baseColor"];updateUniforms(gl,uniformList){gl.uniform1f(uniformList.sceneTransition,this.sliderValues.sceneTransition);var t=1.145-this.sliderValues.wallThickness/10;gl.uniform1f(uniformList.wallThickness,t),gl.uniform1f(uniformList.clipDistance,this.sliderValues.clipDistance),gl.uniform3fv(uniformList.baseColor,this.baseColor)}uiElementsUsed="#wall-thickness-slider, #switch-scene-button, #clip-distance-slider";wallThicknessData=[1.55,-.55,1.55];maxClipDistance=5;doClipBrightening=!0;getNearestCenter(){return[0,0,1,1.875*Math.round(this.cameraPos[3]/1.875)]}getNearestCorner(){var t=1.875,e=(this.cameraPos[3]+t/2)%t-t/2,o=[[1,1,Math.sqrt(3),t/2],[1,-1,Math.sqrt(3),t/2],[-1,1,Math.sqrt(3),t/2],[-1,-1,Math.sqrt(3),t/2],[1,1,Math.sqrt(3),-t/2],[1,-1,Math.sqrt(3),-t/2],[-1,1,Math.sqrt(3),-t/2],[-1,-1,Math.sqrt(3),-t/2]];let s=1/0,r=0;for(let c=0;c<o.length;c++){var a=Math.acosh(this.cameraPos[0]*o[c][0]+this.cameraPos[1]*o[c][1]+this.cameraPos[2]*o[c][2]),i=e-o[c][3],a=a*a+i*i;a<s&&(s=a,r=c)}return[o[r][0],o[r][1],o[r][2],this.cameraPos[3]-e+o[r][3]]}}export{H2xEAxes,H2xERooms};