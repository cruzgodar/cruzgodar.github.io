...
`;class H2xERooms extends H2xEGeometry{distanceEstimatorGlsl=`
	...
	`;uniformNames=["sceneTransition","wallThickness","baseColor"];updateUniforms(gl,uniformList,programIndex){gl.uniform1f(uniformList.sceneTransition[programIndex],this.sliderValues.sceneTransition);var t=1.145-this.sliderValues.wallThickness/10;gl.uniform1f(uniformList.wallThickness[programIndex],t),gl.uniform1f(uniformList.clipDistance[programIndex],this.sliderValues.clipDistance),gl.uniform3fv(uniformList.baseColor[programIndex],this.baseColor)}uiElementsUsed="#wall-thickness-slider, #switch-scene-button, #clip-distance-slider";wallThicknessData=[1.55,-.55,1.55];maxClipDistance=5;doClipBrightening=!0;getNearestCenter(){return[0,0,1,1.875*Math.round(this.cameraPos[3]/1.875)]}getNearestCorner(){var t=1.875,e=(this.cameraPos[3]+t/2)%t-t/2,o=[[1,1,Math.sqrt(3),t/2],[1,-1,Math.sqrt(3),t/2],[-1,1,Math.sqrt(3),t/2],[-1,-1,Math.sqrt(3),t/2],[1,1,Math.sqrt(3),-t/2],[1,-1,Math.sqrt(3),-t/2],[-1,1,Math.sqrt(3),-t/2],[-1,-1,Math.sqrt(3),-t/2]];let r=1/0,s=0;for(let c=0;c<o.length;c++){var a=Math.acosh(this.cameraPos[0]*o[c][0]+this.cameraPos[1]*o[c][1]+this.cameraPos[2]*o[c][2]),i=e-o[c][3],a=a*a+i*i;a<r&&(r=a,s=c)}return[o[s][0],o[s][1],o[s][2],this.cameraPos[3]-e+o[s][3]]}}export{H2xEAxes,H2xERooms};