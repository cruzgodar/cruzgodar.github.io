import{ThurstonGeometry}from"../class.min.js";import{BaseGeometry,getMatrixGlsl,getMinGlslString}from"./base.min.js";import{$}from"/scripts/src/main.min.js";const numericalStepDistance=1e-4,flowNumericallyThreshhold=5e-4,maxNumericalSteps=Math.ceil(flowNumericallyThreshhold/numericalStepDistance),phi=(1+Math.sqrt(5))/2,tauInverse=1/(2*Math.log(phi));function getTransformationMatrix(pos){return[[Math.exp(pos[2]),0,0,pos[0]],[0,Math.exp(-pos[2]),0,pos[1]],[0,0,1,pos[2]],[0,0,0,1]]}function getInverseTransformationMatrix(pos){var t=Math.exp(pos[2]),e=Math.exp(-pos[2]);return[[e,0,0,-e*pos[0]],[0,t,0,-t*pos[1]],[0,0,1,-pos[2]],[0,0,0,1]]}function liftToM(pos){return[phi*pos[0]-pos[1],pos[0]+phi*pos[1],tauInverse*pos[2]]}function getBinarySearchGlslChunk({comparisonVec,dotProductThreshhold,searchIterations}){return`
	...
	`;maxMarches="100";ambientOcclusionDenominator="mix(100.0, 75.0, sceneTransition)";maxT="15.0";cameraPos=[.14282,.17857,-.11161,1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[-.8415,.54025,0,0];forwardVec=[.54025,.8415,0,0];movingSpeed=.5;uniformGlsl=`
		uniform float sceneTransition;
		uniform float wallThickness;
		uniform vec3 baseColor;
	`;uniformNames=["sceneTransition","wallThickness","baseColor"];updateUniforms(gl,uniformList){var t=.372-this.sliderValues.wallThickness/10;gl.uniform1f(uniformList.sceneTransition,this.sliderValues.sceneTransition),gl.uniform1f(uniformList.wallThickness,t),gl.uniform3fv(uniformList.baseColor,this.baseColor)}uiElementsUsed="#wall-thickness-slider, #switch-scene-button";initUI(){var t=$("#wall-thickness-slider"),e=$("#wall-thickness-slider-value");t.min=-.18,t.max=.6,t.value=.3,e.textContent=.3,this.sliderValues.wallThickness=.3}getNearestCenter(){return[0,0,0,1]}getNearestCorner(){var t=[[.2,0,.4,1],[0,.2,.4,1],[-.2,0,.4,1],[0,-.2,.4,1],[.2,0,-.4,1],[0,.2,-.4,1],[-.2,0,-.4,1],[0,-.2,-.4,1]];let e=1/0,o=0;for(let i=0;i<t.length;i++){var a=ThurstonGeometry.magnitude([this.cameraPos[0]-t[i][0],this.cameraPos[1]-t[i][1],this.cameraPos[2]-t[i][2],this.cameraPos[3]-t[i][3]]);a<e&&(e=a,o=i)}return t[o]}}export{SolAxes,SolRooms};