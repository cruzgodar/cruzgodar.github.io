import{BaseGeometry,getMatrixGlsl,getMinGlslString}from"./base.min.js";import{$}from"/scripts/src/main.min.js";const numericalStepDistance=2e-4,flowNumericallyThreshhold=.002,flowNearPlaneThreshhold=1e-4,maxNumericalSteps=Math.ceil(flowNumericallyThreshhold/numericalStepDistance);function getTransformationMatrix(pos){return[[Math.exp(pos[2]),0,0,pos[0]],[0,Math.exp(-pos[2]),0,pos[1]],[0,0,1,pos[2]],[0,0,0,1]]}function getInverseTransformationMatrix(pos){var t=Math.exp(pos[2]),e=Math.exp(-pos[2]);return[[e,0,0,-e*pos[0]],[0,t,0,-t*pos[1]],[0,0,1,-pos[2]],[0,0,0,1]]}const phi=(1+Math.sqrt(5))/2,teleportationElementA1=[phi/(phi+2),-1/(phi+2),0,1],teleportationElementA2=[1/(phi+2),phi/(phi+2),0,1],teleportationElementB=[0,0,2*Math.log(phi),1],teleportationMatrixA1=getTransformationMatrix(teleportationElementA1),teleportationMatrixA1inv=getInverseTransformationMatrix(teleportationElementA1),teleportationMatrixA2=getTransformationMatrix(teleportationElementA2),teleportationMatrixA2inv=getInverseTransformationMatrix(teleportationElementA2),teleportationMatrixB=getTransformationMatrix(teleportationElementB),teleportationMatrixBinv=getInverseTransformationMatrix(teleportationElementB);class SolGeometry extends BaseGeometry{geodesicGlsl=`
