import{ThurstonGeometry}from"../class.min.js";import{BaseGeometry,getMatrixGlsl}from"./base.min.js";import{$}from"/scripts/src/main.min.js";const numericalStepDistance=2e-4,flowNumericallyThreshhold=.002,flowNearPlaneThreshhold=1e-4,maxNumericalSteps=Math.ceil(flowNumericallyThreshhold/numericalStepDistance),phi=(1+Math.sqrt(5))/2,tauInverse=.5*Math.log(phi);function getTransformationMatrix(pos){return[[Math.exp(pos[2]),0,0,pos[0]],[0,Math.exp(-pos[2]),0,pos[1]],[0,0,1,pos[2]],[0,0,0,1]]}function getInverseTransformationMatrix(pos){var t=Math.exp(pos[2]),e=Math.exp(-pos[2]);return[[e,0,0,-e*pos[0]],[0,t,0,-t*pos[1]],[0,0,1,-pos[2]],[0,0,0,1]]}function liftToM(pos){return[phi*pos[0]-pos[1],pos[0]+phi*pos[1],tauInverse*pos[2]]}function getBinarySearchGlslChunk({comparisonVec,dotProductThreshhold,searchIterations}){return`
	...
	`;lightGlsl=`
		...
		lightIntensity = 1.0;
	`;cameraPos=[0,0,0,1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];movingSpeed=.25;uniformGlsl=`
		uniform float wallThickness;
		uniform vec3 baseColor;
	`;uniformNames=["wallThickness","baseColor"];updateUniforms(gl,uniformList){var t=this.sliderValues.wallThickness;gl.uniform1f(uniformList.wallThickness,t),gl.uniform3fv(uniformList.baseColor,this.baseColor)}uiElementsUsed="#wall-thickness-slider";initUI(){var t=$("#wall-thickness-slider"),e=$("#wall-thickness-slider-value");t.min=.33,t.max=.6,t.value=.33,e.textContent=.33,this.sliderValues.wallThickness=.33}}export{SolRooms};