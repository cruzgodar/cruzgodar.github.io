...
function getBinarySearchGlslChunk({
	...
	return /* glsl */`
		...
		if (abs(dotProduct) > ${dotProductThreshhold})
		{
			...
			{
				pos = getUpdatedPos(startPos, rayDirectionVec, oldT + lastTIncrease * currentSearchPosition, g_mu, g_k, g_kPrime, g_m, g_K, g_E, g_L);
...
{
	geodesicGlsl = /* glsl */`
		vec4 pos = getUpdatedPos(startPos, rayDirectionVec, t, g_mu, g_k, g_kPrime, g_m, g_K, g_E, g_L);
	...
	${getBinarySearchGlslChunk({
		...
		globalColor += teleportPos(pos, startPos, rayDirectionVec, t, totalT, g_mu, g_k, g_kPrime, g_m, g_K, g_E, g_L);
	`;
	...
	raymarchSetupGlsl = /* glsl */`
		...
		setGlobals(rayDirectionVec, g_mu, g_k, g_kPrime, g_m, g_K, g_E, g_L);
	`;
	...
	functionGlsl = /* glsl */`
		...
		void runAGMAlgorithm(float g_k, float g_kPrime)
		{
		...
		void setGlobals(vec4 rayDirectionVec, out float g_mu, out float g_k, out float g_kPrime, out float g_m, out float g_K, out float g_E, out float g_L)
		{
		...
		// in this shader.
		float computeJacobiZetaFunction(float tanPhi, float g_m, float g_K, float g_E)
		{
			float t0 = abs(tanPhi);
		...
		vec4 getUpdatedPosExactly(vec4 rayDirectionVec, float t, float g_mu, float g_k, float g_kPrime, float g_m, float g_K, float g_E, float g_L)
		{
			...
			float zeta = computeJacobiZetaFunction(jef1.x / jef1.y, g_m, g_K, g_E) - g_m * jef1.x * jef0.x * jef2.x;
		...
		vec4 getUpdatedPos(vec4 startPos, vec4 rayDirectionVec, float t, float g_mu, float g_k, float g_kPrime, float g_m, float g_K, float g_E, float g_L)
		{
			vec4 pos;
			...
			{
				pos = getUpdatedPosExactly(rayDirectionVec, t, g_mu, g_k, g_kPrime, g_m, g_K, g_E, g_L);
			}
		...
		vec3 teleportPos(inout vec4 pos, inout vec4 startPos, inout vec4 rayDirectionVec, inout float t, inout float totalT, inout float g_mu, inout float g_k, inout float g_kPrime, inout float g_m, inout float g_K, inout float g_E, inout float g_L)
		{
