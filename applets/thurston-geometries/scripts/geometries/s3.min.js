import{ThurstonGeometry}from"../class.min.js";import{sliderValues}from"../index.min.js";import{BaseGeometry,getColorGlslString,getMinGlslString}from"./base.min.js";import{$}from"/scripts/src/main.min.js";class S3Geometry extends BaseGeometry{geodesicGlsl=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,-1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}}function hsvToRgb(h,s,v){function t(n){var t=(n+6*h)%6;return v-v*s*Math.max(0,Math.min(t,Math.min(4-t,1)))}return[255*t(5),255*t(3),255*t(1)]}function getHopfFiber(index,numFibers){var t=index/numFibers*(2*Math.PI),s=Math.PI/2,e=hsvToRgb(s/(2*Math.PI),Math.abs(t%Math.PI-Math.PI/2)/(Math.PI/2),1),t=[Math.cos(t)*Math.sin(s),Math.sin(t)*Math.sin(s),Math.cos(s)],s=ThurstonGeometry.normalize([1+t[2],-t[1],t[0],0]),t=ThurstonGeometry.normalize([0,t[0],t[1],1+t[2]]);return[`
		float distance${index+1} = greatCircleDistance(
			...
			fiberThickness);
		`,e]}class S3HopfFibration extends S3Geometry{constructor(){super();this.distanceEstimatorGlsl="";var t=new Array(20);for(let e=0;e<20;e++){var s=getHopfFiber(e,20);this.distanceEstimatorGlsl+=s[0],t[e]=s[1]}this.distanceEstimatorGlsl+=`
