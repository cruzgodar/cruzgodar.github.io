...
export class E3S2Demo extends E3Geometry
{
	...
	getColorGlsl = /* glsl */`
		...
		float matchCamera = step(distance2, minDistance + .0001) * step(minDistance - .0001, distance2);
	...
	getUpdatedUniforms()
	{
		const uniforms = { cameraDotPos: this.cameraDotPos };

		for (let i = 0; i < this.numRays; i++)
		{
			uniforms[`rayDir${i + 1}`] = this.rayDirs[i];
			uniforms[`testVec${i + 1}`] = this.testVecs[i];
			uniforms[`rayLength${i + 1}`] = this.rayLengths[i];
			uniforms[`rayColor${i + 1}`] = this.rayColors[i];
		}
...
export class S2xES2Demo extends S2xEGeometry
{
	...
	getColorGlsl = /* glsl */`
		...
		float match3 = step(distance3, minDistance + .0001) * step(minDistance - .0001, distance3);
		float match4 = step(distance4, minDistance + .0001) * step(minDistance - .0001, distance4);
