import{ThurstonGeometry}from"../class.min.mjs";import{BaseGeometry,getColorGlslString,getMinGlslString}from"./base.min.mjs";import{$}from"/scripts/src/main.min.mjs";class S3Geometry extends BaseGeometry{geodesicGlsl="vec4 pos = cos(t) * startPos + sin(t) * rayDirectionVec;";fogGlsl="return mix(color, fogColor, 1.0 - exp(-acos(dot(pos, cameraPos)) * fogScaling));";customDotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]+vec1[2]*vec2[2]+vec1[3]*vec2[3]}updateCameraPos(cameraPos,tangentVec,dt){var t=[...cameraPos];for(let s=0;s<4;s++)t[s]=Math.cos(dt)*t[s]+Math.sin(dt)*tangentVec[s];var e=ThurstonGeometry.magnitude(t);return t[0]/=e,t[1]/=e,t[2]/=e,t[3]/=e,t}getGeodesicDirection(pos1,pos2){var t=new Array(4);for(let s=0;s<4;s++)t[s]=(pos2[s]-Math.cos(1)*pos1[s])/Math.sin(1);var e=ThurstonGeometry.magnitude(t);return[ThurstonGeometry.normalize(t),e]}getNormalVec(cameraPos){return ThurstonGeometry.normalize([-cameraPos[0],-cameraPos[1],-cameraPos[2],-cameraPos[3]])}getGammaPrime(_pos,dir){return[...dir]}getGammaDoublePrime(pos){return[-pos[0],-pos[1],-pos[2],-pos[3]]}getGammaTriplePrime(_pos,dir){return[-dir[0],-dir[1],-dir[2],-dir[3]]}gammaTriplePrimeIsLinearlyIndependent=!1}class S3Rooms extends S3Geometry{static distances=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}uniformGlsl="uniform float wallThickness;";uniformNames=["wallThickness"];uniformData={wallThickness:.92};updateUniforms(gl,uniformList){gl.uniform1f(uniformList.wallThickness,this.uniformData.wallThickness)}uiElementsUsed="#wall-thickness-slider";initUI(){const e=$("#wall-thickness-slider"),s=$("#wall-thickness-slider-value");e.value=1e4,s.textContent="0.30",e.addEventListener("input",()=>{var t=.92+.05*(1-parseInt(e.value)/1e4);s.textContent=(Math.round(1e3*(.955-t))/100).toFixed(2),this.uniformData.wallThickness=t})}}class S3Spheres extends S3Geometry{distanceEstimatorGlsl=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,-1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}}function hsvToRgb(h,s,v){function t(n){var t=(n+6*h)%6;return v-v*s*Math.max(0,Math.min(t,Math.min(4-t,1)))}return[255*t(5),255*t(3),255*t(1)]}function getHopfFiber(index,numFibers){var t=index/numFibers*(2*Math.PI),e=Math.PI/2,s=hsvToRgb(e/(2*Math.PI),Math.abs(t%Math.PI-Math.PI/2)/(Math.PI/2),1),t=[Math.cos(t)*Math.sin(e),Math.sin(t)*Math.sin(e),Math.cos(e)],e=ThurstonGeometry.normalize([1+t[2],-t[1],t[0],0]),t=ThurstonGeometry.normalize([0,t[0],t[1],1+t[2]]);return[`float distance${index+1} = greatCircleDistance(
		pos,
		vec4(${e[0]}, ${e[1]}, ${e[2]}, ${e[3]}),
		vec4(${t[0]}, ${t[1]}, ${t[2]}, ${t[3]}),
		fiberThickness);
	`,s]}class S3HopfFibration extends S3Geometry{constructor(){super();this.distanceEstimatorGlsl="";var t=new Array(20);for(let s=0;s<20;s++){var e=getHopfFiber(s,20);this.distanceEstimatorGlsl+=e[0],t[s]=e[1]}this.distanceEstimatorGlsl+=`float minDistance = ${getMinGlslString("distance",20)};`,this.getColorGlsl=this.distanceEstimatorGlsl+getColorGlslString("distance","minDistance",t),this.distanceEstimatorGlsl+="return minDistance;"}functionGlsl=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}uniformGlsl="uniform float fiberThickness;";uniformNames=["fiberThickness"];uniformData={fiberThickness:.025};updateUniforms(gl,uniformList){gl.uniform1f(uniformList.fiberThickness,this.uniformData.fiberThickness)}uiElementsUsed="#fiber-thickness-slider";initUI(){const e=$("#fiber-thickness-slider"),s=$("#fiber-thickness-slider-value");e.value=2105,s.textContent="0.25",e.addEventListener("input",()=>{var t=.005+parseInt(e.value)/1e4*.095;s.textContent=(Math.round(1e3*t)/100).toFixed(2),this.uniformData.fiberThickness=t})}}export{S3Rooms,S3Spheres,S3HopfFibration};