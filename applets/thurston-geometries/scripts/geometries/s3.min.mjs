import{ThurstonGeometry}from"../class.min.mjs";import{sliderValues}from"../index.min.mjs";import{BaseGeometry,getColorGlslString,getMinGlslString}from"./base.min.mjs";import{$}from"/scripts/src/main.min.mjs";class S3Geometry extends BaseGeometry{geodesicGlsl="vec4 pos = cos(t) * startPos + sin(t) * rayDirectionVec;";fogGlsl="return mix(color, fogColor, 1.0 - exp(-acos(dot(pos, cameraPos)) * fogScaling));";customDotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]+vec1[2]*vec2[2]+vec1[3]*vec2[3]}updateCameraPos(cameraPos,tangentVec,dt){var t=[...cameraPos];for(let e=0;e<4;e++)t[e]=Math.cos(dt)*t[e]+Math.sin(dt)*tangentVec[e];var s=ThurstonGeometry.magnitude(t);return t[0]/=s,t[1]/=s,t[2]/=s,t[3]/=s,t}getGeodesicDistance(pos1,pos2,dir){var t=[pos2[0]-pos1[0],pos2[1]-pos1[1],pos2[2]-pos1[2],pos2[3]-pos1[3]];return Math.sqrt(this.dotProduct(t,t))}getNormalVec(cameraPos){return this.normalize([-cameraPos[0],-cameraPos[1],-cameraPos[2],-cameraPos[3]])}getGammaPrime(_pos,dir){return[...dir]}getGammaDoublePrime(pos){return[-pos[0],-pos[1],-pos[2],-pos[3]]}getGammaTriplePrime(_pos,dir){return[-dir[0],-dir[1],-dir[2],-dir[3]]}gammaTriplePrimeIsLinearlyIndependent=!1}class S3Rooms extends S3Geometry{static distances=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}uniformGlsl="uniform float wallThickness;";uniformNames=["wallThickness"];updateUniforms(gl,uniformList){var t=.97-(sliderValues.wallThickness- -.15)/.5*(.97-.92);gl.uniform1f(uniformList.wallThickness,t)}uiElementsUsed="#wall-thickness-slider";initUI(){var t=$("#wall-thickness-slider"),s=$("#wall-thickness-slider-value");t.min=-.15,t.max=.35,t.value=.35,s.textContent=.35.toFixed(3),sliderValues.wallThickness=.35}}class S3Spheres extends S3Geometry{distanceEstimatorGlsl=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,-1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}}function hsvToRgb(h,s,v){function t(n){var t=(n+6*h)%6;return v-v*s*Math.max(0,Math.min(t,Math.min(4-t,1)))}return[255*t(5),255*t(3),255*t(1)]}function getHopfFiber(index,numFibers){var t=index/numFibers*(2*Math.PI),s=Math.PI/2,e=hsvToRgb(s/(2*Math.PI),Math.abs(t%Math.PI-Math.PI/2)/(Math.PI/2),1),t=[Math.cos(t)*Math.sin(s),Math.sin(t)*Math.sin(s),Math.cos(s)],s=this.normalize([1+t[2],-t[1],t[0],0]),t=this.normalize([0,t[0],t[1],1+t[2]]);return[`float distance${index+1} = greatCircleDistance(
		pos,
		vec4(${s[0]}, ${s[1]}, ${s[2]}, ${s[3]}),
		vec4(${t[0]}, ${t[1]}, ${t[2]}, ${t[3]}),
		fiberThickness);
	`,e]}class S3HopfFibration extends S3Geometry{constructor(){super();this.distanceEstimatorGlsl="";var t=new Array(20);for(let e=0;e<20;e++){var s=getHopfFiber(e,20);this.distanceEstimatorGlsl+=s[0],t[e]=s[1]}this.distanceEstimatorGlsl+=`float minDistance = ${getMinGlslString("distance",20)};`,this.getColorGlsl=this.distanceEstimatorGlsl+getColorGlslString("distance","minDistance",t),this.distanceEstimatorGlsl+="return minDistance;"}functionGlsl=`
