import{ThurstonGeometry}from"../class.min.mjs";import{BaseGeometry,getColorGlslString,getMinGlslString}from"./base.min.mjs";class S3Geometry extends BaseGeometry{geodesicGlsl="vec4 pos = cos(t) * cameraPos + sin(t) * rayDirectionVec;";fogGlsl="return mix(color, fogColor, 1.0 - exp(-acos(dot(pos, cameraPos)) * fogScaling));";customDotProduct(vec1,vec2){return vec1[0]*vec2[0]+vec1[1]*vec2[1]+vec1[2]*vec2[2]+vec1[3]*vec2[3]}updateCameraPos(cameraPos,tangentVec,dt){var t=[...cameraPos];for(let e=0;e<4;e++)t[e]=Math.cos(dt)*t[e]+Math.sin(dt)*tangentVec[e];var o=ThurstonGeometry.magnitude(t);return t[0]/=o,t[1]/=o,t[2]/=o,t[3]/=o,t}getGeodesicDirection(pos1,pos2){var t=new Array(4);for(let e=0;e<4;e++)t[e]=(pos2[e]-Math.cos(1)*pos1[e])/Math.sin(1);var o=ThurstonGeometry.magnitude(t);return[ThurstonGeometry.normalize(t),o]}getNormalVec(cameraPos){return ThurstonGeometry.normalize([-cameraPos[0],-cameraPos[1],-cameraPos[2],-cameraPos[3]])}getGammaPrime(_pos,dir){return[...dir]}getGammaDoublePrime(pos){return[-pos[0],-pos[1],-pos[2],-pos[3]]}getGammaTriplePrime(_pos,dir){return[-dir[0],-dir[1],-dir[2],-dir[3]]}gammaTriplePrimeIsLinearlyIndependent=!1}class S3Rooms extends S3Geometry{static distances=`
	...
	${getColorGlslString("distance","minDistance",[[255,0,0],[0,255,255],[0,255,0],[255,0,255],[0,0,255],[255,255,0],[255,127,0],[127,0,255]])};
	`;lightGlsl=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,-1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}}function hsvToRgb(h,s,v){function t(n){var t=(n+6*h)%6;return v-v*s*Math.max(0,Math.min(t,Math.min(4-t,1)))}return[255*t(5),255*t(3),255*t(1)]}function getHopfFiber(index,numFibers){var t=index/numFibers*(2*Math.PI),o=Math.PI/2,e=hsvToRgb(o/(2*Math.PI),Math.abs(t%Math.PI-Math.PI/2)/(Math.PI/2),1),t=[Math.cos(t)*Math.sin(o),Math.sin(t)*Math.sin(o),Math.cos(o)],o=ThurstonGeometry.normalize([1+t[2],-t[1],t[0],0]),t=ThurstonGeometry.normalize([0,t[0],t[1],1+t[2]]);return[`float distance${index+1} = greatCircleDistance(
		pos,
		vec4(${o[0]}, ${o[1]}, ${o[2]}, ${o[3]}),
		vec4(${t[0]}, ${t[1]}, ${t[2]}, ${t[3]}),
		.025);
	`,e]}class S3HopfFibration extends S3Geometry{constructor(){super();this.distanceEstimatorGlsl="";var t=new Array(20);for(let e=0;e<20;e++){var o=getHopfFiber(e,20);this.distanceEstimatorGlsl+=o[0],t[e]=o[1]}this.distanceEstimatorGlsl+=`float minDistance = ${getMinGlslString("distance",20)};`,this.getColorGlsl=this.distanceEstimatorGlsl+getColorGlslString("distance","minDistance",t),this.distanceEstimatorGlsl+="return minDistance;"}functionGlsl=`
	...
	`;cameraPos=[0,0,0,-1];normalVec=[0,0,0,1];upVec=[0,0,1,0];rightVec=[0,1,0,0];forwardVec=[1,0,0,0];getMovingSpeed(){return 1}}export{S3Rooms,S3Spheres,S3HopfFibration};