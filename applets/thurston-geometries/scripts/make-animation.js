...
export class ThurstonGeometry extends Applet
{
	...
	drawFrame(timestamp)
	{
		do
		{
			...
			if (this.needNewFrame)
			{
				if (frame >= startFrame)
				{
					this.wilson.downloadFrame(String(frame).padStart(6, "0") + ".png");
				}
				frame++;
			...
			if (!this.animationPaused && frame < 30000)
			{
				if (frame >= startFrame)
				{
					setTimeout(() =>
	...
	changeResolution(resolution = this.resolution)
	{
		...
		this.wilson.worldWidth = 2 * ratio;
