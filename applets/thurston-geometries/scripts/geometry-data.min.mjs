import{ThurstonGeometry}from"./class.min.mjs";function getE3BaseData(){return{geodesicGlsl:"vec4 pos = cameraPos + t * rayDirectionVec;",fogGlsl:"return mix(color, fogColor, 1.0 - exp(-length(pos - cameraPos) * fogScaling));",updateCameraPos:(cameraPos,tangentVec,dt)=>{for(let t=0;t<4;t++)cameraPos[t]=cameraPos[t]+dt*tangentVec[t]},getNormalVec:()=>[0,0,0,1],getGammaPrime:(pos,dir)=>[-pos[0]+dir[0],-pos[1]+dir[1],-pos[2]+dir[2],-pos[3]+dir[3]],getGammaDoublePrime:()=>[0,0,0,0],cameraPos:[0,0,0,1],normalVec:[0,0,0,1],upVec:[0,0,1,0],rightVec:[0,1,0,0],forwardVec:[1,0,0,0],movingSpeed:2}}function getS3BaseData(){return{geodesicGlsl:"vec4 pos = cos(t) * cameraPos + sin(t) * rayDirectionVec;",fogGlsl:"return mix(color, fogColor, 1.0 - exp(-acos(dot(pos, cameraPos)) * fogScaling));",updateCameraPos:(cameraPos,tangentVec,dt)=>{for(let a=0;a<4;a++)cameraPos[a]=Math.cos(dt)*cameraPos[a]+Math.sin(dt)*tangentVec[a];var t=ThurstonGeometry.magnitude(cameraPos);cameraPos[0]/=t,cameraPos[1]/=t,cameraPos[2]/=t,cameraPos[3]/=t},getNormalVec:cameraPos=>ThurstonGeometry.normalize(cameraPos),getGammaPrime:(_pos,dir)=>[...dir],getGammaDoublePrime:pos=>[-pos[0],-pos[1],-pos[2],-pos[3]],cameraPos:[0,0,0,-1],normalVec:[0,0,0,-1],upVec:[0,0,1,0],rightVec:[0,1,0,0],forwardVec:[1,0,0,0],movingSpeed:1}}function getE3SpheresData(){return{...getE3BaseData(),distanceEstimatorGlsl:`
		...
		`}}function getS3SpheresData(){return{...getS3BaseData(),distanceEstimatorGlsl:`
		...
		`}}export{getE3SpheresData,getS3RoomsData,getS3SpheresData};