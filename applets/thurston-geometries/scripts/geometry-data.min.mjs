import{ThurstonGeometry}from"./class.min.mjs";function getE3BaseData(){return{geodesicGlsl:"vec4 pos = cameraPos + t * rayDirectionVec;",fogGlsl:"return mix(color, fogColor, 1.0 - exp(-length(pos - cameraPos) * fogScaling));",updateCameraPos:(cameraPos,tangentVec,dt)=>{for(let t=0;t<4;t++)cameraPos[t]=cameraPos[t]+dt*tangentVec[t]},getNormalVec:()=>[0,0,0,1],getGammaPrime:(_pos,dir)=>[...dir],getGammaDoublePrime:()=>[0,0,0,0],getGammaTriplePrime:()=>[0,0,0,0]}}function getS3BaseData(){return{geodesicGlsl:"vec4 pos = cos(t) * cameraPos + sin(t) * rayDirectionVec;",fogGlsl:"return mix(color, fogColor, 1.0 - exp(-acos(dot(pos, cameraPos)) * fogScaling));",updateCameraPos:(cameraPos,tangentVec,dt)=>{for(let a=0;a<4;a++)cameraPos[a]=Math.cos(dt)*cameraPos[a]+Math.sin(dt)*tangentVec[a];var t=ThurstonGeometry.magnitude(cameraPos);cameraPos[0]/=t,cameraPos[1]/=t,cameraPos[2]/=t,cameraPos[3]/=t},getNormalVec:cameraPos=>ThurstonGeometry.normalize([-cameraPos[0],-cameraPos[1],-cameraPos[2],-cameraPos[3]]),getGammaPrime:(_pos,dir)=>[...dir],getGammaDoublePrime:pos=>[-pos[0],-pos[1],-pos[2],-pos[3]],getGammaTriplePrime:(_pos,dir)=>[-dir[0],-dir[1],-dir[2],-dir[3]]}}function getH3BaseData(){return{geodesicGlsl:"vec4 pos = cosh(t) * cameraPos + sinh(t) * rayDirectionVec;",fogGlsl:`return mix(
			color,
			fogColor,
			0.0/*1.0 - exp(-acosh(-dot(pos, cameraPos)) * fogScaling)*/
		);`,updateCameraPos:(cameraPos,tangentVec,dt)=>{for(let a=0;a<4;a++)cameraPos[a]=Math.cosh(dt)*cameraPos[a]+Math.sinh(dt)*tangentVec[a];var t=cameraPos[0]*cameraPos[0]+cameraPos[1]*cameraPos[1]+cameraPos[2]*cameraPos[2]-cameraPos[3]*cameraPos[3];cameraPos[0]/=-t,cameraPos[1]/=-t,cameraPos[2]/=-t,cameraPos[3]/=-t},getNormalVec:cameraPos=>ThurstonGeometry.normalize([cameraPos[0],cameraPos[1],cameraPos[2],-cameraPos[3]]),getGammaPrime:(_pos,dir)=>[...dir],getGammaDoublePrime:pos=>[...pos],getGammaTriplePrime:(_pos,dir)=>[...dir]}}function getE3RoomsData(){return{...getE3BaseData(),distanceEstimatorGlsl:`
