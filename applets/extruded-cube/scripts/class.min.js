import{RaymarchApplet}from"/scripts/applets/raymarchApplet.min.js";class ExtrudedCube extends RaymarchApplet{constructor({canvas}){super({canvas:canvas,distanceEstimatorGlsl:`
		...
		`,uniforms:{iterations:["int",16],scale:["float",3],separation:["float",1]},cameraPos:[2.0538,2.0548,2.0354],theta:1.25*Math.PI,phi:2.1539})}distanceEstimator(x,y,z){var a=this.uniforms.scale[1],s=this.uniforms.separation[1],t=this.uniforms.iterations[1],e=(a+1)/(a-1)*s;let o=[Math.abs(x),Math.abs(y),Math.abs(z)],i=Math.max(Math.max(o[0],o[1]),o[2])-1;for(let r=0;r<t;r++)o=o[0]>Math.max(o[1],o[2])?[a*o[0]-(a-1)*e,a*o[1],a*o[2]]:o[1]>Math.max(o[0],o[2])?[a*o[0],a*o[1]-(a-1)*e,a*o[2]]:[a*o[0],a*o[1],a*o[2]-(a-1)*e],o=[Math.abs(o[0]),Math.abs(o[1]),Math.abs(o[2])],i=Math.min(i,(Math.max(Math.max(o[0],o[1]),o[2])-1)/Math.pow(a,r+1));return Math.abs(i)}}export{ExtrudedCube};