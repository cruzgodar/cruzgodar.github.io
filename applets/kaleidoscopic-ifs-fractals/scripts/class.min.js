import{getVectorGlsl}from"/scripts/applets/applet.min.js";import{dotProduct,getRotationMatrix,mat3TimesVector,RaymarchApplet}from"/scripts/applets/raymarchApplet.min.js";import{changeOpacity}from"/scripts/src/animation.min.js";const ns={tetrahedron:[[-.57735,0,.816496],[.288675,-.5,.816496],[.288675,.5,.816496]],cube:[[1,0,0],[0,1,0],[0,0,1]],octahedron:[[.707107,0,.707107],[0,.707107,.707107],[-.707107,0,.707107],[0,-.707107,.707107]],dodecahedron:[[.809016,-.5,.309017],[.309017,.809016,-.5],[-.5,.309017,.809016],[.809016,-.5,.309017],[.309017,.809016,-.5],[-.5,.309017,.809016],[.809016,-.5,.309017]],icosahedron:[[0,1,0],[-.5,-.309017,.809016],[.5,-.309017,.809016],[-.809016,.5,.309017],[.809016,.5,.309017],[0,1,0]]},scaleCenters={tetrahedron:[0,0,1],cube:[.57735,.57735,.57735],octahedron:[0,0,1],dodecahedron:[.57735,.57735,.57735],icosahedron:[0,.525731,.850651]};function getDistanceEstimatorGlsl(shape,useForGetColor=!1){const t=shape.charAt(0).toUpperCase()+shape.slice(1);var e=ns[shape].length,e=Array(e).fill(0).map((_,i)=>useForGetColor?`
	...
	`}class KaleidoscopicIFSFractal extends RaymarchApplet{rotationAngleX=0;rotationAngleY=0;rotationAngleZ=0;shape="octahedron";constructor({canvas,shape="octahedron"}){var t=[];for(const a in ns){const r=a.charAt(0).toUpperCase()+a.slice(1);var e=Array(ns[a].length).fill(0).map((_,i)=>`
					const vec3 n${i}${r} = ${getVectorGlsl(ns[a][i])};
				`).join("\n")+`
				const vec3 scaleCenter${r} = ${getVectorGlsl(scaleCenters[a])};
			`;t.push(e)}var o=`
			...
			${t.join("\n")}
		`;super({canvas:canvas,distanceEstimatorGlsl:getDistanceEstimatorGlsl(shape),getColorGlsl:getDistanceEstimatorGlsl(shape,!0),addGlsl:o,uniforms:{scale:["float",2],rotationMatrix:["mat3",[[1,0,0],[0,1,0],[0,0,1]]],numIterations:["int",56]},theta:.2004,phi:1.6538,cameraPos:[-2.03816,-.526988,.30503],lightPos:[-50,-70,100],lightBrightness:1.25,epsilonScaling:.75,stepFactor:.6}),this.shape=shape}distanceEstimator(x,y,z){var e=this.uniforms.scale[1],o=ns[this.shape??"octahedron"],a=scaleCenters[this.shape??"octahedron"],r=this.uniforms.rotationMatrix[1];for(let t=0;t<56;t++){for(let t=0;t<o.length;t++){var s=dotProduct([x,y,z],o[t]);s<0&&(x-=2*s*o[t][0],y-=2*s*o[t][1],z-=2*s*o[t][2])}x=e*x-(e-1)*a[0],y=e*y-(e-1)*a[1],z=e*z-(e-1)*a[2],[x,y,z]=mat3TimesVector(r,[x,y,z])}return Math.sqrt(x*x+y*y+z*z)*Math.pow(e,-56)}updateMatrices(){this.setUniform("rotationMatrix",getRotationMatrix(this.rotationAngleX,this.rotationAngleY,this.rotationAngleZ)),this.needNewFrame=!0}async changePolyhedron(newShape){await changeOpacity({element:this.canvas,opacity:0}),this.shape=newShape;var t=getDistanceEstimatorGlsl(this.shape),e=getDistanceEstimatorGlsl(this.shape,!0);this.reloadShader({distanceEstimatorGlsl:t,getColorGlsl:e}),await changeOpacity({element:this.canvas,opacity:1})}}export{KaleidoscopicIFSFractal};