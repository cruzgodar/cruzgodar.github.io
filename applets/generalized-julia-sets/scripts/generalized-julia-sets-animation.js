!function()
{
	...
	let code_string = "cadd(csub(cpow(z, 2.0), vec2(0.0, cmul(.05, cpow(z, -2.0).y))), c)";
	
	let a = 0;
	let b = 0;
	let image_size = 4000;
	let num_iterations = 75;
	let brightness_scale = 9.1032;
	
	let small_canvas_size = 0;
	let large_canvas_size = 0;

	let small_image_size = 4000;

	let small_num_iterations = 75;
	let large_num_iterations = 20750;
	...
	function draw_frame()
	{
		...
		if (num_pixels_at_max < 1.5 * image_size && brightness_scale > .25)
		{
			brightness_scale -= .1 / (a*a + b*b + 2);
		}
		...
		link.href = document.querySelector("#julia-set").toDataURL();
		...
		a = .5 * (Math.cos(2 * 2 * Math.PI * frame / 6000) + Math.sin(5 * 2 * Math.PI * frame / 6000)) - .75;
		b = .5 * (Math.cos(2 * Math.PI * frame / 6000) + Math.sin(3 * 2 * Math.PI * frame / 6000));
