import{RaymarchingFundamentals}from"./class.min.js";import{extrudedCubeDE,mengerSpongeDE}from"./distanceEstimators.min.js";class CubeAndSponge extends RaymarchingFundamentals{sphereWeight=0;extrudedCubeWeight=1;extrudedCubeSeparation=1.5;mengerSpongeWeight=0;mengerSpongeScale=3;constructor({canvas,useShadows=!0,useReflections=!1}){var e=`
		...
		`;console.log(e),super({canvas:canvas,fragShaderSource:e,uniforms:["sphereWeight","extrudedCubeSeparation","extrudedCubeWeight","mengerSpongeWeight","mengerSpongeScale"]}),this.wilson.gl.uniform1f(this.wilson.uniforms.sphereWeight,this.sphereWeight),this.wilson.gl.uniform1f(this.wilson.uniforms.extrudedCubeWeight,this.extrudedCubeWeight),this.wilson.gl.uniform1f(this.wilson.uniforms.extrudedCubeSeparation,this.extrudedCubeSeparation),this.wilson.gl.uniform1f(this.wilson.uniforms.mengerSpongeWeight,this.mengerSpongeWeight),this.wilson.gl.uniform1f(this.wilson.uniforms.mengerSpongeScale,this.mengerSpongeScale)}distanceEstimator(x,y,z){return console.log(this.distanceEstimatorExtrudedCube(x,y,z),this.distanceEstimatorMengerSponge(x,y,z)),this.extrudedCubeWeight*this.distanceEstimatorExtrudedCube(x,y,z)+this.mengerSpongeWeight*this.distanceEstimatorMengerSponge(x,y,z)}distanceEstimatorExtrudedCube(x,y,z){var e=(this.extrudedCubeScale+1)/(this.extrudedCubeScale-1)*this.extrudedCubeSeparation;let t=[3*Math.abs(x),3*Math.abs(y),3*Math.abs(z)-2.5],o=Math.max(Math.max(t[0],t[1]),t[2])-1;for(let a=0;a<this.iterations;a++)t=t[0]>Math.max(t[1],t[2])?[this.extrudedCubeScale*t[0]-(this.extrudedCubeScale-1)*e,this.extrudedCubeScale*t[1],this.extrudedCubeScale*t[2]]:t[1]>Math.max(t[0],t[2])?[this.extrudedCubeScale*t[0],this.extrudedCubeScale*t[1]-(this.extrudedCubeScale-1)*e,this.extrudedCubeScale*t[2]]:[this.extrudedCubeScale*t[0],this.extrudedCubeScale*t[1],this.extrudedCubeScale*t[2]-(this.extrudedCubeScale-1)*e],t=[Math.abs(t[0]),Math.abs(t[1]),Math.abs(t[2])],o=Math.min(o,(Math.max(Math.max(t[0],t[1]),t[2])-1)/Math.pow(this.extrudedCubeScale,a+1));return Math.abs(o)/3}distanceEstimatorMengerSponge(x,y,z){let e=[3*Math.abs(x),3*Math.abs(y),3*Math.abs(z)-2.5];var t=Math.max(Math.max(e[0],e[1]),e[2]),o=Math.min(Math.min(e[0],e[1]),e[2]),a=e[0]+e[1]+e[2];e=[o,a-o-t,t];let i;var n=[1,1,1];let s;var r=1/this.mengerSpongeScale,c=2*this.mengerSpongeScale/(+this.mengerSpongeScale-1),l=2*this.mengerSpongeScale/(this.mengerSpongeScale-1),m=[(c-1)*((+this.mengerSpongeScale+1)/(1+2*this.mengerSpongeScale-+this.mengerSpongeScale)),(c-1)*((+this.mengerSpongeScale+1)/(1+2*this.mengerSpongeScale-+this.mengerSpongeScale)),(c-1)*((+this.mengerSpongeScale+1)/(1+2*this.mengerSpongeScale-+this.mengerSpongeScale))],d=[0,l-1,l-1],h=.5*(1-r),u=.5*(1+r),p=.5*(1-r),g=.5*(1+r);for(let E=0;E<16;E++){var f=Math.abs(e[0]-u)-h,S=Math.abs(e[1]-u)-h,b=Math.abs(e[2]-u)-h,v=Math.max(f,Math.max(S,b)),C=e[0]-r,M=Math.abs(e[1]-g)-p,w=Math.abs(e[2]-g)-p,D=Math.max(C,Math.max(M,w));v<D?(i=v,s=f>Math.max(S,b)?n[0]:S>Math.max(f,b)?n[1]:n[2],e=[c*e[0]-m[0],c*e[1]-m[1],c*e[2]-m[2]],n[0]*=c,n[1]*=c,n[2]*=c):(i=D,s=C>Math.max(M,w)?n[0]:M>Math.max(C,w)?n[1]:n[2],e=[this.mengerSpongeScale*e[0]-d[0],l*e[1]-d[1],l*e[2]-d[2]],n[0]*=this.mengerSpongeScale,n[1]*=l,n[2]*=l),e=[Math.abs(e[0]),Math.abs(e[1]),Math.abs(e[2])],t=Math.max(Math.max(e[0],e[1]),e[2]),o=Math.min(Math.min(e[0],e[1]),e[2]),a=e[0]+e[1]+e[2],e=[o,a-o-t,t]}return Math.abs(i)/s*.333333}}export{CubeAndSponge};