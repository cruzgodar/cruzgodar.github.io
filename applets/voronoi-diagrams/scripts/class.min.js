import anime from"/scripts/anime.min.js";import{Applet,getMinGlslString,getVectorGlsl}from"/scripts/src/applets.min.js";import{Wilson}from"/scripts/wilson.min.js";class VoronoiDiagram extends Applet{wilsonHidden;lastTimestamp=-1;currentlyAnimating=!1;numPoints=20;metric=2;resolution=1e3;resolutionHidden=100;maximumSpeed;t;radius;maxRadius;pointRadius;pointOpacity;points;colors;cancelAnimaton=()=>{};constructor({canvas}){super(canvas);var i=`
	...
	${this.points.map((point,index)=>`
		...
		`}generatePoints(){this.points=new Array(this.numPoints);for(let r=0;r<this.numPoints;r++)this.points[r]=[.9*(Math.random()-.5)*this.wilson.worldWidth,.9*(Math.random()-.5)*this.wilson.worldHeight];var t,n=new Array(this.numPoints),i=.2/this.numPoints;for(let a=0;a<this.numPoints;a++){n[a]=[0,0];for(let i=0;i<this.numPoints;i++)i!==a&&(t=(this.points[i][0]-this.points[a][0])**2+(this.points[i][1]-this.points[a][1])**2,n[a][0]+=(this.points[a][0]-this.points[i][0])/t,n[a][1]+=(this.points[a][1]-this.points[i][1])/t)}for(let l=0;l<this.numPoints;l++)this.points[l][0]+=i*n[l][0],this.points[l][1]+=i*n[l][1];for(let d=0;d<2;d++){var s,o,e=d%2?1:-1;for(let t=0;t<this.numPoints;t++)for(let i=0;i<this.numPoints;i++)i!==t&&(s=Math.abs(this.points[t][0]-this.points[i][0]),o=Math.abs(this.points[t][1]-this.points[i][1]),s<.01&&(this.points[t][0]+=.01*e),o<.01&&(this.points[t][1]+=.01*e),Math.abs(s-o)<.01)&&(this.points[t][0]+=.02*e)}for(let h=0;h<this.numPoints;h++)this.points[h][0]=Math.min(Math.max(this.points[h][0],-this.wilson.worldWidth/2),this.wilson.worldWidth/2),this.points[h][1]=Math.min(Math.max(this.points[h][1],-this.wilson.worldHeight/2),this.wilson.worldHeight/2);this.colors=new Array(this.numPoints);for(let c=0;c<this.numPoints;c++)this.colors[c]=this.wilson.utils.hsvToRgb(Math.random(),.5+.25*Math.random(),.5+.5*Math.random()),this.colors[c][0]/=255,this.colors[c][1]/=255,this.colors[c][2]/=255}findMaxRadius(){let i=.5;let t=.25;for(let s=0;s<10;s++){var n=2*i;this.testRadius(n)?9!==s&&(i-=t):i+=t,t/=2}return 2*i+.025}testRadius(radius){this.wilsonHidden.gl.uniform1f(this.wilsonHidden.uniforms.radius,radius),this.wilsonHidden.render.drawFrame();var i=this.wilsonHidden.render.getPixelData();for(let t=0;t<i.length;t+=4)if(0===i[t]&&0===i[t+1]&&0===i[t+2])return!1;return!0}drawFrame(){this.radius=this.t*this.maxRadius,this.wilson.gl.uniform1f(this.wilson.uniforms.radius,this.radius),this.wilson.gl.uniform1f(this.wilson.uniforms.pointOpacity,this.pointOpacity),this.wilson.render.drawFrame()}updateMetric(){this.cancelAnimaton(),this.t=2,this.wilson.gl.uniform1f(this.wilson.uniforms.metric,this.metric),this.drawFrame()}}export{VoronoiDiagram};