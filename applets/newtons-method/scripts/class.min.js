import{hsvToRgb,rgbToHex}from"../../../scripts/applets/applet.min.js";import anime from"/scripts/anime.min.js";import{AnimationFrameApplet}from"/scripts/applets/animationFrameApplet.min.js";import{WilsonGPU}from"/scripts/wilson.min.js";class NewtonsMethod extends AnimationFrameApplet{wilsonHidden;rootSetterElement;rootAInput;rootBInput;colorSetterElement;colors={root0:[216/255,1/255,42/255],root1:[1,139/255,56/255],root2:[249/255,239/255,20/255],root3:[27/255,181/255,61/255],root4:[0,86/255,195/255],root5:[154/255,82/255,164/255],root6:[32/255,32/255,32/255],root7:[155/255,92/255,15/255]};lastActiveRoot="root0";numRoots=3;numIterations=100;secantProportion=0;pastBrightnessScales=[];resolution=500;resolutionHidden=50;constructor({canvas,rootSetterElement,rootAInput,rootBInput,colorSetterElement}){super(canvas),this.rootSetterElement=rootSetterElement,this.rootAInput=rootAInput,this.rootBInput=rootBInput,this.colorSetterElement=colorSetterElement;var o=this.createHiddenCanvas(),t=(this.randomizeColors(),{shader:`
		...
		`,uniforms:{worldCenter:[0,0],worldSize:[2,2],numRoots:this.numRoots,root0:[0,0],root1:[0,0],root2:[0,0],root3:[0,0],root4:[0,0],root5:[0,0],root6:[0,0],root7:[0,0],color0:this.colors.root0,color1:this.colors.root1,color2:this.colors.root2,color3:this.colors.root3,color4:this.colors.root4,color5:this.colors.root5,color6:this.colors.root6,color7:this.colors.root7,a:[1,0],c:[0,0],brightnessScale:12.75,secantProportion:this.secantProportion},canvasWidth:this.resolution,worldWidth:4,minWorldWidth:1e-5,maxWorldWidth:100,minWorldHeight:1e-5,maxWorldHeight:100,onResizeCanvas:()=>this.needNewFrame=!0,interactionOptions:{useForPanAndZoom:!0,onPanAndZoom:()=>this.needNewFrame=!0},draggableOptions:{draggables:{a:[1,0],c:[0,0],root0:[0,0],root1:[0,0],root2:[0,0],root3:[0,0],root4:[0,0],root5:[0,0],root6:[0,0],root7:[0,0]},callbacks:{drag:this.onDragDraggable.bind(this),release:this.onReleaseDraggable.bind(this)}},fullscreenOptions:{onSwitch:this.switchFullscreen.bind(this),beforeSwitch:this.beforeSwitchFullscreen.bind(this),fillScreen:!0,useFullscreenButton:!0,enterFullscreenButtonIconPath:"/graphics/general-icons/enter-fullscreen.png",exitFullscreenButtonIconPath:"/graphics/general-icons/exit-fullscreen.png"}});this.wilson=new WilsonGPU(canvas,t),this.wilsonHidden=new WilsonGPU(o,{...t,canvasWidth:this.resolutionHidden});for(let s=3;s<8;s++)this.wilson.draggables["root"+s].element.style.display="none";this.spreadRoots(!0),this.resume()}switchMethod(instant){const o={t:this.secantProportion};var t=0===this.secantProportion?1:0;anime({targets:o,t:t,duration:instant?10:1e3,easing:"easeInOutQuad",update:()=>{this.secantProportion=o.t,this.wilson.setUniforms({secantProportion:this.secantProportion}),this.wilsonHidden.setUniforms({secantProportion:this.secantProportion}),this.needNewFrame=!0}})}addRoot(){var o,t;8!==this.numRoots&&(o=3*Math.random()-1.5,t=3*Math.random()-1.5,this.wilson.setDraggables({["root"+this.numRoots]:[o,t]}),this.wilson.draggables["root"+this.numRoots].element.style.display="block",this.wilson.setUniforms({["root"+this.numRoots]:this.wilson.draggables["root"+this.numRoots].location}),this.wilsonHidden.setUniforms({["root"+this.numRoots]:this.wilson.draggables["root"+this.numRoots].location}),this.numRoots++,this.wilson.setUniforms({numRoots:this.numRoots}),this.wilsonHidden.setUniforms({numRoots:this.numRoots}),this.needNewFrame=!0)}removeRoot(){1!==this.numRoots&&(this.numRoots--,this.wilson.draggables["root"+this.numRoots].element.style.display="none",this.wilson.setUniforms({numRoots:this.numRoots}),this.wilsonHidden.setUniforms({numRoots:this.numRoots}),this.needNewFrame=!0)}spreadRoots(noAnimation=!1,randomize=!1){const e=Object.fromEntries(Object.entries(this.wilson.draggables).map(([id,draggable])=>[id,draggable.location])),r={};for(let t=0;t<8;t++){var o=1+.75*randomize*Math.random();r["root"+t]=[o*Math.cos(2*Math.PI*t/this.numRoots),o*Math.sin(2*Math.PI*t/this.numRoots)]}const i={t:0};anime({targets:i,t:1,duration:noAnimation?10:1e3,easing:"easeInOutQuad",update:()=>{for(const s of Object.keys(r)){var o=e[s],t=r[s],o=[(1-i.t)*o[0]+i.t*t[0],(1-i.t)*o[1]+i.t*t[1]];this.wilson.setDraggables({[s]:o}),this.wilson.setUniforms({[s]:o}),this.wilsonHidden.setUniforms({[s]:o})}this.needNewFrame=!0},complete:()=>{for(const o of Object.keys(r))this.wilson.setDraggables({[o]:r[o]}),this.wilson.setUniforms({[o]:r[o]}),this.wilsonHidden.setUniforms({[o]:r[o]});this.needNewFrame=!0}})}setRoot(x,y){this.wilson.setDraggables({[this.lastActiveRoot]:[x,y]}),this.wilson.setUniforms({[this.lastActiveRoot]:[x,y]}),this.wilsonHidden.setUniforms({[this.lastActiveRoot]:[x,y]}),this.needNewFrame=!0}setColor(rgb,root=this.lastActiveRoot){if(root in this.colors){var o=rgb[0]/255,t=rgb[1]/255,s=rgb[2]/255;const e={r:this.colors[root][0],g:this.colors[root][1],b:this.colors[root][2]},r="color"+root.slice(4);anime({targets:e,r:o,g:t,b:s,easing:"easeInOutQuad",duration:250,update:()=>{this.colors[root][0]=e.r,this.colors[root][1]=e.g,this.colors[root][2]=e.b,this.wilson.setUniforms({[r]:this.colors[root]}),this.wilsonHidden.setUniforms({[r]:this.colors[root]}),this.needNewFrame=!0}})}}randomizeColors(){for(const t in this.colors){var o=hsvToRgb((.55*Math.random()+.525)%1,.2*Math.random()+.3,.2*Math.random()+.8);this.setColor(o,t)}}onDragDraggable({id,x,y}){this.wilson.setUniforms({[id]:[x,y]}),this.wilsonHidden.setUniforms({[id]:[x,y]}),this.needNewFrame=!0}onReleaseDraggable({id}){var o;this.lastActiveRoot=id,this.rootSetterElement&&this.colorSetterElement&&(this.rootAInput.setValue(Math.round(1e3*this.wilson.draggables[this.lastActiveRoot].location[0])/1e3,!1),this.rootBInput.setValue(Math.round(1e3*this.wilson.draggables[this.lastActiveRoot].location[1])/1e3,!1),this.lastActiveRoot in this.colors)&&(o=this.colors[this.lastActiveRoot],this.colorSetterElement.firstElementChild.value=rgbToHex(255*o[0],255*o[1],255*o[2]))}drawFrame(){this.wilsonHidden.setUniforms({worldSize:[this.wilson.worldWidth,this.wilson.worldHeight],worldCenter:[this.wilson.worldCenterX,this.wilson.worldCenterY]}),this.wilsonHidden.drawFrame();var o=this.wilsonHidden.readPixels(),t=new Array(this.resolutionHidden*this.resolutionHidden);for(let r=0;r<this.resolutionHidden*this.resolutionHidden;r++)t[r]=Math.max(Math.max(o[4*r],o[4*r+1]),o[4*r+2]);t.sort((a,b)=>a-b);let s=Math.min(7e3/(t[Math.floor(this.resolutionHidden*this.resolutionHidden*.96)]+t[Math.floor(this.resolutionHidden*this.resolutionHidden*.98)]),30);this.pastBrightnessScales.push(s);var e=this.pastBrightnessScales.length;10<e&&this.pastBrightnessScales.shift();for(let i=s=0;i<this.pastBrightnessScales.length;i++)s+=this.pastBrightnessScales[i];s=Math.max(s/e,.5),this.wilson.setUniforms({worldSize:[this.wilson.worldWidth,this.wilson.worldHeight],worldCenter:[this.wilson.worldCenterX,this.wilson.worldCenterY],brightnessScale:s}),this.wilson.drawFrame()}switchFullscreen(){this.resume()}async beforeSwitchFullscreen(){this.animationPaused=!0,await new Promise(resolve=>setTimeout(resolve,33))}}export{NewtonsMethod};