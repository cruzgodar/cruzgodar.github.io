import{getFloatGlsl,tempShader}from"../../../scripts/applets/applet.min.js";import{AnimationFrameApplet}from"/scripts/applets/animationFrameApplet.min.js";import{doubleEncodingGlsl,loadGlsl}from"/scripts/src/complexGlsl.min.js";import{sleep}from"/scripts/src/utils.min.js";import{WilsonGPU}from"/scripts/wilson.min.js";class KickedRotator extends AnimationFrameApplet{resolution=1e3;computeResolution=1e3;wilsonUpdate;loadPromise;texture;imageData;maxBrightness=1;frame;numIterations;constructor({canvas}){super(canvas);var t=this.createHiddenCanvas(),e={shader:tempShader,canvasWidth:this.computeResolution,worldCenterX:Math.PI,worldCenterY:Math.PI,worldWidth:2*Math.PI,verbose:window.DEBUG},t=(this.wilsonUpdate=new WilsonGPU(t,e),{shader:`
		...
		`;this.texture=new Float32Array(this.computeResolution*this.computeResolution*4),this.imageData=new Float32Array(this.resolution*this.resolution*4),this.maxBrightness=1,this.wilson.setUniforms({maxBrightness:1});for(let i=0;i<this.computeResolution;i++)for(let t=0;t<this.computeResolution;t++){var e=this.computeResolution*i+t,s=this.wilsonUpdate.interpolateCanvasToWorld([i,t]);this.texture[4*e]=(s[0]-Math.PI)/22+Math.PI+.2*(Math.random()-.5),this.texture[4*e+1]=s[1],this.texture[4*e+2]=0,this.texture[4*e+3]=1}this.wilsonUpdate.loadShader({id:"update",shader:t}),this.wilsonUpdate.setTexture({id:"update2",data:this.texture}),this.frame=0,this.numIterations=100,this.resume()}prepareFrame(){this.frame++,this.needNewFrame=this.frame<this.numIterations}drawFrame(){var t=this.frame%2==0?"update1":"update2",e=this.frame%2==0?"update2":"update1",s=(this.wilsonUpdate.useTexture(t),this.wilsonUpdate.useFramebuffer(e),this.wilsonUpdate.drawFrame(),this.wilsonUpdate.readPixels({format:"float"}));for(let a=0;a<this.computeResolution;a++)for(let t=0;t<this.computeResolution;t++){var i=this.computeResolution*a+t,o=Math.round(((s[4*i+1]-this.wilsonUpdate.worldCenterY)/this.wilsonUpdate.worldHeight+.5)*this.resolution),i=Math.round(((s[4*i]-this.wilsonUpdate.worldCenterX)/this.wilsonUpdate.worldWidth+.5)*this.resolution);0<=o&&o<this.resolution&&0<=i&&i<this.resolution&&(o=o*this.resolution+i,this.imageData[4*o]++,this.maxBrightness=Math.max(this.maxBrightness,this.imageData[4*o]))}this.wilson.setTexture({id:"output",data:this.imageData});t=Math.min(this.frame/15,1);this.wilson.setUniforms({maxBrightness:this.maxBrightness/t}),this.wilson.drawFrame()}switchFullscreen(){this.resume()}async beforeSwitchFullscreen(){this.pause(),await sleep(33)}}export{KickedRotator};