!function()
{
	...
	function grade_submission(entry_string)
	{
		...
		console.log(words_by_score[words_by_score.length - 1][0], words_by_score[words_by_score.length - 2][0], words_by_score[words_by_score.length - 3][0], words_by_score[words_by_score.length - 4][0], words_by_score[words_by_score.length - 5][0]);
		
		console.log(valid_solutions.length);
		...
		let refresh_id = setInterval(() =>
		{
			...
			confirm_buttons[active_row - 1].firstElementChild.textContent = Math.round(100 * Math.pow(Math.sin(Math.PI / 2 * t / 120), .35) * entry_score) / 100;
		...
		else if (entry_score !== 100)
		{
			setTimeout(() =>
			{
				best_buttons[active_row - 1].style.opacity = 1;
				best_buttons[active_row - 1].style.cursor = "pointer";
			}, 1800);
		}
	...
	let valid_guesses = JSON.parse(JSON.stringify(all_guesses));
	let valid_solutions = JSON.parse(JSON.stringify(all_solutions));
}()