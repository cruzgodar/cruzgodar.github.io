"use strict";class VectorField extends Applet{load_promise=null;resolution=500;num_particles=0;max_particles=5e3;aspect_ratio=1;zoom_level=.6515;fixed_point_x=0;fixed_point_y=0;dt=.0075;lifetime=100;last_timestamp=-1;particles=[];free_particle_slots=[];update_texture=null;dim_texture=null;update_canvas=null;dim_canvas=null;wilson_update=null;wilson_dim=null;pan_velocity_x=0;pan_velocity_y=0;zoom_velocity=0;next_pan_velocity_x=0;next_pan_velocity_y=0;next_zoom_velocity=0;last_pan_velocities_x=[];last_pan_velocities_y=[];last_zoom_velocities=[];pan_friction=.96;pan_velocity_start_threshhold=25e-5;pan_velocity_stop_threshhold=25e-5;zoom_friction=.9;zoom_velocity_start_threshhold=.002;zoom_velocity_stop_threshhold=.002;constructor(i){super(i),this.update_canvas=document.createElement("canvas"),this.update_canvas.classList.add("hidden-canvas"),this.hidden_canvases.push(this.update_canvas),Page.element.appendChild(this.update_canvas),this.dim_canvas=document.createElement("canvas"),this.dim_canvas.classList.add("hidden-canvas"),this.hidden_canvases.push(this.dim_canvas),Page.element.appendChild(this.dim_canvas);let t={renderer:"gpu",shader:"precision highp float; varying vec2 uv; void main(void) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }",canvas_width:100,canvas_height:100};this.wilson_update=new Wilson(this.update_canvas,t),this.wilson_update.render.create_framebuffer_texture_pair(),this.wilson_update.gl.bindTexture(this.wilson_update.gl.TEXTURE_2D,this.wilson_update.render.framebuffers[0].texture),this.wilson_update.gl.bindFramebuffer(this.wilson_update.gl.FRAMEBUFFER,null);let e=`
