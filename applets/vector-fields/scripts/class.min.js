import{doubleEncodingGlsl,getGlslBundle,loadGlsl}from"../../../scripts/src/complexGlsl.min.js";import{AnimationFrameApplet}from"/scripts/applets/animationFrameApplet.min.js";import{getEqualPixelFullScreen,getFloatGlsl,getMaxGlslString,tempShader}from"/scripts/applets/applet.min.js";import{addTemporaryListener}from"/scripts/src/main.min.js";import{Wilson}from"/scripts/wilson.min.js";class VectorFields extends AnimationFrameApplet{loadPromise;resolution=500;numParticles=0;maxParticles=5e3;loopEdges=!1;particleDilation;aspectRatio=1;zoomLevel=.6515;fixedPointX=0;fixedPointY=0;dt=.00375;lifetime=150;particles=[];freeParticleSlots=[];updateTexture;dimTexture;updateCanvas;dimCanvas;wilsonUpdate;wilsonDim;panVelocityX=0;panVelocityY=0;zoomVelocity=0;nextPanVelocityX=0;nextPanVelocityY=0;nextZoomVelocity=0;lastPanVelocitiesX=[];lastPanVelocitiesY=[];lastZoomVelocities=[];panFriction=.96;panVelocityStartThreshhold=25e-5;panVelocityStopThreshhold=25e-5;zoomFriction=.9;zoomVelocityStartThreshhold=.002;zoomVelocityStopThreshhold=.002;timeElapsedHistoryLength=60;lastTimeElapsed=new Array(this.timeElapsedHistoryLength);averageTimeElapsed;frame=0;drawFrameCallback=()=>{};constructor({canvas,draggable1Location=[1,0],draggable2Location=[-1,0],loopEdges=!1}){super(canvas),this.loopEdges=loopEdges,this.updateCanvas=this.createHiddenCanvas(),this.dimCanvas=this.createHiddenCanvas();this.wilsonUpdate=new Wilson(this.updateCanvas,{renderer:"gpu",shader:tempShader,canvasWidth:100,canvasHeight:100}),this.wilsonUpdate.render.createFramebufferTexturePair(),this.wilsonUpdate.gl.bindTexture(this.wilsonUpdate.gl.TEXTURE_2D,this.wilsonUpdate.render.framebuffers[0].texture),this.wilsonUpdate.gl.bindFramebuffer(this.wilsonUpdate.gl.FRAMEBUFFER,null);var i={renderer:"gpu",shader:`
		...
		`;this.wilsonUpdate.render.shaderPrograms=[],this.wilsonUpdate.render.loadNewShader(t),this.wilsonUpdate.render.loadNewShader(e),this.wilsonUpdate.render.loadNewShader(s),this.wilsonUpdate.render.loadNewShader(o),this.wilsonUpdate.render.loadNewShader(i);for(let a=0;a<5;a++)this.wilsonUpdate.render.initUniforms(["dt","draggableArg","draggableArg2"],a),this.wilsonUpdate.gl.useProgram(this.wilsonUpdate.render.shaderPrograms[a]),this.wilsonUpdate.gl.uniform1f(this.wilsonUpdate.uniforms.dt[a],this.dt),this.wilsonUpdate.gl.uniform2fv(this.wilsonUpdate.uniforms.draggableArg[a],this.wilson.draggables.worldCoordinates[0]),this.wilsonUpdate.gl.uniform2fv(this.wilsonUpdate.uniforms.draggableArg2[a],this.wilson.draggables.worldCoordinates[1]);t=`
			precision highp float;
			precision highp sampler2D;
		...
		`;this.wilson.render.loadNewShader(t),this.wilson.render.initUniforms(["maxBrightness","stepSizeX","stepSizeY"]),this.wilson.gl.uniform1f(this.wilson.uniforms.maxBrightness,this.lifetime/255),this.wilson.draggables.draggables[0].style.display=-1!==generatingCode.indexOf("draggableArg")?"block":"none",this.wilson.draggables.draggables[1].style.display=-1!==generatingCode.indexOf("draggableArg2")?"block":"none",this.generateNewField({resolution:resolution,maxParticles:maxParticles,dt:dt,lifetime:lifetime,worldCenterX:worldCenterX,worldCenterY:worldCenterY,zoomLevel:zoomLevel})}getSamplingGlsl(){var t=this.particleDilation??Math.floor(this.resolution/500);if(0===t)return`
