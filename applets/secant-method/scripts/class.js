	...
	spreadRoots(noAnimation = false, randomize = false)
	{
		...
		anime({
			...
			update: () =>
			{
				for (let i = 0; i < this.numRoots; i++)
				{
					this.currentRoots[2 * i] = (1 - dummy.t) * oldRoots[2 * i]
						+ dummy.t * newRoots[2 * i];

					this.currentRoots[2 * i + 1] = (1 - dummy.t) * oldRoots[2 * i + 1]
	...
	async onReleaseDraggable(activeDraggable)
	{
		...
		)
		{
			...
			else
			{
				...
				this.rootAInputElement.value = Math.round(
					this.currentRoots[2 * index] * 1000
				) / 1000;

				this.rootBInputElement.value = Math.round(
					this.currentRoots[2 * index + 1] * 1000
				) / 1000;
