import{VectorField}from"/applets/vector-fields/scripts/class.min.js";import{Applet}from"/scripts/applets/applet.min.js";import{RaymarchApplet}from"/scripts/applets/raymarchApplet.min.js";import{addTemporaryListener,pageElement}from"/scripts/src/main.min.js";import{Wilson}from"/scripts/wilson.min.js";class HairyBall extends RaymarchApplet{theta=Math.PI;phi=Math.PI/2;lockedOnOrigin=!0;distanceFromOrigin=1.55;cameraPos=[this.distanceFromOrigin,0,0];vectorFieldApplet;constructor({canvas}){super(canvas);var i=this.createHiddenCanvas(),i=(i.style.display="block",i.classList.remove("hidden-canvas"),i.classList.add("output-canvas"),pageElement.appendChild(i),this.vectorFieldApplet=new VectorField({canvas:i}),this.vectorFieldApplet.drawFrameCallback=this.drawFrame.bind(this),this.vectorFieldApplet.loadPromise.then(()=>{this.vectorFieldApplet.run({generatingCode:"(sin(0.5*(y+3.14159))*cos(x), -sin(0.5*(y+3.14159))*sin(x))",resolution:this.imageSize,maxParticles:1e4,dt:.00375,lifetime:150,worldCenterX:0,worldCenterY:0,zoomLevel:.6515})}),{renderer:"gpu",shader:`
