import{AnimationFrameApplet}from"/scripts/applets/animationFrameApplet.min.js";import{siteSettings}from"/scripts/src/settings.min.js";import{WilsonGPU}from"/scripts/wilson.min.js";class AbelianSandpile extends AnimationFrameApplet{numGrains=1e4;floodGrains=0;resolution=319;computationsPerFrame=20;lastPixelData;constructor({canvas}){super(canvas);var e={shaders:{init:`
		...
		`},uniforms:{init:{stepSize:0,startGrains:[0,0,0,0],floodGrains:[0,0,0,0]},update:{stepSize:0},draw:{color1:[0,0,0],color2:[0,0,0],color3:[0,0,0]}},canvasWidth:this.resolution,reduceMotion:siteSettings.reduceMotion,fullscreenOptions:{useFullscreenButton:!0,enterFullscreenButtonIconPath:"/graphics/general-icons/enter-fullscreen.png",exitFullscreenButtonIconPath:"/graphics/general-icons/exit-fullscreen.png"}};this.wilson=new WilsonGPU(canvas,e),this.canvas.style.imageRendering="pixelated"}run({resolution=100,numGrains=1e4,floodGrains=0,computationsPerFrame=25,palette=[[229,190,237],[149,147,217],[124,144,219]]}){this.resolution=resolution,this.numGrains=numGrains,this.floodGrains=floodGrains,this.computationsPerFrame=computationsPerFrame;var e=[Math.floor(this.numGrains/16777216)%256/256,Math.floor(this.numGrains/65536)%256/256,Math.floor(this.numGrains/256)%256/256,this.numGrains%256/256];this.wilson.setUniform({shader:"init",name:"stepSize",value:1/this.resolution}),this.wilson.setUniform({shader:"init",name:"startGrains",value:e}),this.wilson.setUniform({shader:"init",name:"floodGrains",value:[0,0,0,this.floodGrains/256]}),this.wilson.setUniform({shader:"update",name:"stepSize",value:1/this.resolution}),this.wilson.setUniform({shader:"draw",name:"color1",value:[palette[0][0]/255,palette[0][1]/255,palette[0][2]/255]}),this.wilson.setUniform({shader:"draw",name:"color2",value:[palette[1][0]/255,palette[1][1]/255,palette[1][2]/255]}),this.wilson.setUniform({shader:"draw",name:"color3",value:[palette[2][0]/255,palette[2][1]/255,palette[2][2]/255]}),this.wilson.resizeCanvas({width:this.resolution}),this.wilson.createFramebufferTexturePair({id:"0",textureType:"float"}),this.wilson.createFramebufferTexturePair({id:"1",textureType:"float"}),this.wilson.useTexture("0"),this.wilson.useFramebuffer(null),this.wilson.useShader("update"),this.wilson.setTexture({id:"0",data:null}),this.wilson.setTexture({id:"1",data:null}),this.wilson.useShader("init"),this.wilson.useTexture("1"),this.wilson.useFramebuffer("0"),this.wilson.drawFrame(),this.wilson.useTexture("0"),this.resume()}drawFrame(){this.wilson.useShader("update");for(let e=0;e<this.computationsPerFrame;e++)this.wilson.useFramebuffer("1"),this.wilson.drawFrame(),this.wilson.useTexture("1"),this.wilson.useFramebuffer("0"),this.wilson.drawFrame(),this.wilson.useTexture("0");this.wilson.useShader("draw"),this.wilson.useFramebuffer(null),this.wilson.drawFrame();var s=this.wilson.readPixels();if(this.lastPixelData){let e=!1;for(let t=Math.floor(s.length/2-4);t<s.length;t++)if(s[t]!==this.lastPixelData[t]){e=!0;break}if(!e)return void this.pause()}this.lastPixelData=s,this.needNewFrame=!0}}export{AbelianSandpile};