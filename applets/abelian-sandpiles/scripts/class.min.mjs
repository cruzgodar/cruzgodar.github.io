import{Applet}from"/scripts/src/applets.min.mjs";import{Wilson}from"/scripts/wilson.min.mjs";class AbelianSandpile extends Applet{wilsonUpscale=null;numGrains=1e4;resolution=500;lastTimestamp=-1;computationsPerFrame=20;lastPixelData=null;constructor(canvas){super(canvas);var hiddenCanvas=this.createHiddenCanvas(),options={renderer:"gpu",shader:`
		...
		`,canvasWidth:this.resolution,canvasHeight:this.resolution},hiddenCanvas=(this.wilson=new Wilson(hiddenCanvas,options),this.wilson.render.loadNewShader(`
			precision highp float;
			precision highp sampler2D;
		...
		`,canvasWidth:this.resolution,canvasHeight:this.resolution,useFullscreen:!0,useFullscreenButton:!0,enterFullscreenButtonIconPath:"/graphics/general-icons/enter-fullscreen.png",exitFullscreenButtonIconPath:"/graphics/general-icons/exit-fullscreen.png"});this.wilsonUpscale=new Wilson(canvas,hiddenCanvas),this.wilsonUpscale.render.createFramebufferTexturePair(this.wilsonUpscale.gl.UNSIGNED_BYTE)}run({numGrains=1e4,computationsPerFrame=25}){this.resume(),this.numGrains=numGrains,this.resolution=Math.floor(Math.sqrt(this.numGrains))+2,this.resolution=this.resolution+1-this.resolution%2,this.computationsPerFrame=computationsPerFrame;var numGrains=this.numGrains%256/256,computationsPerFrame=Math.floor(this.numGrains/256)%256/256,grains2=Math.floor(this.numGrains/65536)%256/256,grains1=Math.floor(this.numGrains/16777216)%256/256,grains1=(this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[0]),this.wilson.gl.uniform1f(this.wilson.uniforms.step[0],1/this.resolution),this.wilson.gl.uniform4f(this.wilson.uniforms.startGrains[0],grains1,grains2,computationsPerFrame,numGrains),this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[1]),this.wilson.gl.uniform1f(this.wilson.uniforms.step[1],1/this.resolution),this.wilson.changeCanvasSize(this.resolution,this.resolution),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[0].texture),this.wilson.gl.texImage2D(this.wilson.gl.TEXTURE_2D,0,this.wilson.gl.RGBA,this.wilson.canvasWidth,this.wilson.canvasHeight,0,this.wilson.gl.RGBA,this.wilson.gl.FLOAT,null),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[1].texture),this.wilson.gl.texImage2D(this.wilson.gl.TEXTURE_2D,0,this.wilson.gl.RGBA,this.wilson.canvasWidth,this.wilson.canvasHeight,0,this.wilson.gl.RGBA,this.wilson.gl.FLOAT,null),Math.max(this.resolution,this.canvas.getBoundingClientRect().width));this.wilsonUpscale.gl.bindTexture(this.wilsonUpscale.gl.TEXTURE_2D,this.wilsonUpscale.render.framebuffers[0].texture),this.wilsonUpscale.changeCanvasSize(grains1,grains1),this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[0]),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[0].texture),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,this.wilson.render.framebuffers[0].framebuffer),this.wilson.render.drawFrame(),window.requestAnimationFrame(this.drawFrame.bind(this))}drawFrame(timestamp){var timeElapsed=timestamp-this.lastTimestamp;if(this.lastTimestamp=timestamp,0!=timeElapsed){this.wilson.gl.viewport(0,0,this.resolution,this.resolution);for(let i=0;i<this.computationsPerFrame;i++)this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[1]),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,this.wilson.render.framebuffers[1].framebuffer),this.wilson.render.drawFrame(),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[1].texture),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,this.wilson.render.framebuffers[0].framebuffer),this.wilson.render.drawFrame(),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[0].texture);this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[2]),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,null),this.wilson.render.drawFrame();var pixelData=this.wilson.render.getPixelData();if(null!==this.lastPixelData){let foundDiff=!1;for(let i=0;i<pixelData.length;i++)if(pixelData[i]!==this.lastPixelData[i]){foundDiff=!0;break}if(!foundDiff)return}this.lastPixelData=pixelData,this.wilsonUpscale.gl.texImage2D(this.wilsonUpscale.gl.TEXTURE_2D,0,this.wilsonUpscale.gl.RGBA,this.wilson.canvasWidth,this.wilson.canvasHeight,0,this.wilsonUpscale.gl.RGBA,this.wilsonUpscale.gl.UNSIGNED_BYTE,pixelData),this.wilsonUpscale.gl.bindFramebuffer(this.wilsonUpscale.gl.FRAMEBUFFER,null),this.wilsonUpscale.render.drawFrame(),this.animationPaused||window.requestAnimationFrame(this.drawFrame.bind(this))}}}export{AbelianSandpile};