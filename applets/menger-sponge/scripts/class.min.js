...
`;function getDistanceEstimatorGlsl(useForGetColor=!1){return`
	...
	`}class MengerSponge extends RaymarchApplet{rotationAngleX=0;rotationAngleY=0;rotationAngleZ=0;constructor({canvas}){super({canvas:canvas,distanceEstimatorGlsl:getDistanceEstimatorGlsl(),getColorGlsl:getDistanceEstimatorGlsl(!0),addGlsl:`
		...
		`,uniforms:{scale:3,iterations:16,rotationMatrix:[[1,0,0],[0,1,0],[0,0,1]]},sceneOrigin:[3.5,0,0],theta:3*Math.PI/2,phi:Math.PI/2,epsilonScaling:1.25,stepFactor:.75,lightBrightness:1.75,overstepFactor:1.4})}distanceEstimator(x,y,z){var e=this.uniforms.scale,a=this.uniforms.iterations;let o=[Math.abs(x),Math.abs(y),Math.abs(z)];var t=Math.max(Math.max(o[0],o[1]),o[2]),s=Math.min(Math.min(o[0],o[1]),o[2]),r=o[0]+o[1]+o[2];o=[s,r-s-t,t];let n;var c=[1,1,1];let i;var l=1/e,d=2*e/(e-1),m=2*e/(e-1),f=[d-1,d-1,d-1],p=[0,m-1,m-1],g=.5*(1-l),h=.5*(1+l),b=.5*(1-l),S=.5*(1+l);for(let P=0;P<a;P++){var v=Math.abs(o[0]-h)-g,u=Math.abs(o[1]-h)-g,M=Math.abs(o[2]-h)-g,C=Math.max(v,Math.max(u,M)),T=o[0]-l,A=Math.abs(o[1]-S)-b,E=Math.abs(o[2]-S)-b,F=Math.max(T,Math.max(A,E));C<F?(n=C,i=v>Math.max(u,M)?c[0]:u>Math.max(v,M)?c[1]:c[2],o=[d*o[0]-f[0],d*o[1]-f[1],d*o[2]-f[2]],c[0]*=d,c[1]*=d,c[2]*=d):(n=F,i=T>Math.max(A,E)?c[0]:A>Math.max(T,E)?c[1]:c[2],o=[e*o[0]-p[0],m*o[1]-p[1],m*o[2]-p[2]],c[0]*=e,c[1]*=m,c[2]*=m),o=[Math.abs(o[0]),Math.abs(o[1]),Math.abs(o[2])],o=mat3TimesVector(this.uniforms.rotationMatrix,o),t=Math.max(Math.max(o[0],o[1]),o[2]),s=Math.min(Math.min(o[0],o[1]),o[2]),r=o[0]+o[1]+o[2],o=[s,r-s-t,t]}return Math.abs(n)/i}}export{MengerSponge};