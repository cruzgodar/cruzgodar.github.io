			...
			float distanceEstimator(vec3 pos)
			{
				...
				for (int iteration = 0; iteration < maxIterations; iteration++)
				{
					...
					float distanceToEdge = max(
						...
						invScale - minAbsPos.y
					);

					if (distanceToCorner < distanceToEdge)
					{
						totalDistance = distanceToCorner / pow(scale, float(iteration + 1));

						// Scale all directions by 2s/(s-1) from (1, 1, 1).
						float factor = 2.0 * scale / (scale - 1.0);
						mutablePos = factor * mutablePos - (factor - 1.0) * vec3(1.0, 1.0, 1.0);
					}
