...
`;function getDistanceEstimatorGlsl(useForGetColor=!1){return`
	...
	`}class MengerSponge extends RaymarchApplet{rotationAngleX=0;rotationAngleY=0;rotationAngleZ=0;constructor({canvas,xrFramebufferScaleSlider}){super({canvas:canvas,distanceEstimatorGlsl:getDistanceEstimatorGlsl(),getColorGlsl:getDistanceEstimatorGlsl(!0),addGlsl:`
			const int maxIterations = 16;
		`,uniformsGlsl:`
			uniform float scale;
			uniform int iterations;
			uniform mat3 rotationMatrix;
		`,uniforms:{scale:3,iterations:16,rotationMatrix:[[1,0,0],[0,1,0],[0,0,1]]},sceneOrigin:[4.5,0,0],theta:3*Math.PI/2,phi:Math.PI/2,epsilonScalingFactor:.5,maxMarches:512,stepFactor:.5,lightBrightness:2.5,overstepFactor:1,xrFramebufferScaleSlider:xrFramebufferScaleSlider,coneMarchingScales:[16,4],coneMarchingMaxMarches:[96,48]})}distanceEstimator(x,y,z){var e=this.uniforms.scale,a=this.uniforms.iterations;let o=[Math.abs(x),Math.abs(y),Math.abs(z)];var t=Math.max(Math.max(o[0],o[1]),o[2]),s=Math.min(Math.min(o[0],o[1]),o[2]),r=o[0]+o[1]+o[2];o=[s,r-s-t,t];let c;var n=[1,1,1];let i;var l=1/e,d=2*e/(e-1),m=2*e/(e-1),f=[d-1,d-1,d-1],g=[0,m-1,m-1],p=.5*(1-l),h=.5*(1+l),S=.5*(1-l),b=.5*(1+l);for(let P=0;P<a;P++){var M=Math.abs(o[0]-h)-p,u=Math.abs(o[1]-h)-p,v=Math.abs(o[2]-h)-p,C=Math.max(M,Math.max(u,v)),T=o[0]-l,A=Math.abs(o[1]-b)-S,F=Math.abs(o[2]-b)-S,E=Math.max(T,Math.max(A,F));C<E?(c=C,i=M>Math.max(u,v)?n[0]:u>Math.max(M,v)?n[1]:n[2],o=[d*o[0]-f[0],d*o[1]-f[1],d*o[2]-f[2]],n[0]*=d,n[1]*=d,n[2]*=d):(c=E,i=T>Math.max(A,F)?n[0]:A>Math.max(T,F)?n[1]:n[2],o=[e*o[0]-g[0],m*o[1]-g[1],m*o[2]-g[2]],n[0]*=e,n[1]*=m,n[2]*=m),o=[Math.abs(o[0]),Math.abs(o[1]),Math.abs(o[2])],o=mat3TimesVector(this.uniforms.rotationMatrix,o),t=Math.max(Math.max(o[0],o[1]),o[2]),s=Math.min(Math.min(o[0],o[1]),o[2]),r=o[0]+o[1]+o[2],o=[s,r-s-t,t]}return Math.abs(c)/i}}export{MengerSponge};