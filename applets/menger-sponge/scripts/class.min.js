import{getRotationMatrix,mat3TimesVector,RaymarchApplet}from"/scripts/applets/raymarchApplet.min.js";const changeColorGlsl=`
...
`;function getDistanceEstimatorGlsl(useForGetColor=!1){return`
	...
	`}class MengerSponge extends RaymarchApplet{rotationAngleX=0;rotationAngleY=0;rotationAngleZ=0;constructor({canvas}){super({canvas:canvas,distanceEstimatorGlsl:getDistanceEstimatorGlsl(),getColorGlsl:getDistanceEstimatorGlsl(!0),addGlsl:`
		...
		`,uniforms:{scale:["float",3],iterations:["int",16],rotationMatrix:["mat3",[[1,0,0],[0,1,0],[0,0,1]]]},cameraPos:[2.1978,1.4366,1.5174],fogColor:[.6,.73,.87],theta:3.7518,phi:2.1482,stepFactor:.5,epsilonScaling:1.75,lightBrightness:1.75})}distanceEstimator(x,y,z){var e=this.uniforms.scale[1],a=this.uniforms.iterations[1];let o=[Math.abs(x),Math.abs(y),Math.abs(z)];var t=Math.max(Math.max(o[0],o[1]),o[2]),s=Math.min(Math.min(o[0],o[1]),o[2]),r=o[0]+o[1]+o[2];o=[s,r-s-t,t];let n;var c=[1,1,1];let i;var l=1/e,d=2*e/(e-1),m=2*e/(e-1),g=[d-1,d-1,d-1],f=[0,m-1,m-1],p=.5*(1-l),h=.5*(1+l),b=.5*(1-l),S=.5*(1+l);for(let P=0;P<a;P++){var M=Math.abs(o[0]-h)-p,v=Math.abs(o[1]-h)-p,C=Math.abs(o[2]-h)-p,T=Math.max(M,Math.max(v,C)),u=o[0]-l,A=Math.abs(o[1]-S)-b,E=Math.abs(o[2]-S)-b,F=Math.max(u,Math.max(A,E));T<F?(n=T,i=M>Math.max(v,C)?c[0]:v>Math.max(M,C)?c[1]:c[2],o=[d*o[0]-g[0],d*o[1]-g[1],d*o[2]-g[2]],c[0]*=d,c[1]*=d,c[2]*=d):(n=F,i=u>Math.max(A,E)?c[0]:A>Math.max(u,E)?c[1]:c[2],o=[e*o[0]-f[0],m*o[1]-f[1],m*o[2]-f[2]],c[0]*=e,c[1]*=m,c[2]*=m),o=[Math.abs(o[0]),Math.abs(o[1]),Math.abs(o[2])],o=mat3TimesVector(this.uniforms.rotationMatrix[1],o),t=Math.max(Math.max(o[0],o[1]),o[2]),s=Math.min(Math.min(o[0],o[1]),o[2]),r=o[0]+o[1]+o[2],o=[s,r-s-t,t]}return Math.abs(n)/i}updateMatrices(){this.setUniform("rotationMatrix",getRotationMatrix(this.rotationAngleX,this.rotationAngleY,this.rotationAngleZ)),this.needNewFrame=!0}}export{MengerSponge};