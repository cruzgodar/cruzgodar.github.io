import{Applet}from"/scripts/applets/applet.min.js";import{RaymarchApplet}from"/scripts/applets/raymarchApplet.min.js";import{addTemporaryListener}from"/scripts/src/main.min.js";import{Wilson}from"/scripts/wilson.min.js";const changeColorGlsl=`
...
`;function getDistanceEstimatorGlsl(useForGetColor=!1){return`
	...
	`}class MengerSponge extends RaymarchApplet{iterations=16;scale=3;separation=1;cameraPos=[2.016,1.3095,1.3729];theta=3.7518;phi=2.1482;constructor({canvas}){super(canvas);var e={renderer:"gpu",shader:`
		...
		`,canvasWidth:500,canvasHeight:500,worldCenterX:-this.theta,worldCenterY:-this.phi,useFullscreen:!0,trueFullscreen:!0,useFullscreenButton:!0,enterFullscreenButtonIconPath:"/graphics/general-icons/enter-fullscreen.png",exitFullscreenButtonIconPath:"/graphics/general-icons/exit-fullscreen.png",switchFullscreenCallback:this.changeResolution.bind(this),mousedownCallback:this.onGrabCanvas.bind(this),touchstartCallback:this.onGrabCanvas.bind(this),mousedragCallback:this.onDragCanvas.bind(this),touchmoveCallback:this.onDragCanvas.bind(this),mouseupCallback:this.onReleaseCanvas.bind(this),touchendCallback:this.onReleaseCanvas.bind(this)};this.wilson=new Wilson(canvas,e),this.wilson.render.initUniforms(["aspectRatioX","aspectRatioY","imageSize","cameraPos","imagePlaneCenterPos","forwardVec","rightVec","upVec","distanceToScene","iterations","scale"]),this.calculateVectors(),this.imageWidth>=this.imageHeight?(this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioX,this.imageWidth/this.imageHeight),this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioY,1)):(this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioX,1),this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioY,this.imageWidth/this.imageHeight)),this.wilson.gl.uniform1i(this.wilson.uniforms.imageSize,this.imageSize),this.wilson.gl.uniform3fv(this.wilson.uniforms.cameraPos,this.cameraPos),this.wilson.gl.uniform3fv(this.wilson.uniforms.imagePlaneCenterPos,this.imagePlaneCenterPos),this.wilson.gl.uniform3fv(this.wilson.uniforms.forwardVec,this.forwardVec),this.wilson.gl.uniform3fv(this.wilson.uniforms.rightVec,this.rightVec),this.wilson.gl.uniform3fv(this.wilson.uniforms.upVec,this.upVec),this.wilson.gl.uniform1f(this.wilson.uniforms.distanceToScene,this.distanceToScene),this.wilson.gl.uniform1i(this.wilson.uniforms.iterations,this.iterations),this.wilson.gl.uniform1f(this.wilson.uniforms.scale,this.scale);addTemporaryListener({object:window,event:"resize",callback:()=>this.changeResolution()}),this.resume()}prepareFrame(timeElapsed){this.pan.update(timeElapsed),this.zoom.update(timeElapsed),this.moveUpdate(timeElapsed)}drawFrame(){this.wilson.worldCenterY=Math.min(Math.max(this.wilson.worldCenterY,.01-Math.PI),-.01),this.theta=-this.wilson.worldCenterX,this.phi=-this.wilson.worldCenterY,this.wilson.render.drawFrame()}distanceEstimator(x,y,z){let e=[Math.abs(x),Math.abs(y),Math.abs(z)];var t=Math.max(Math.max(e[0],e[1]),e[2]),a=Math.min(Math.min(e[0],e[1]),e[2]),s=e[0]+e[1]+e[2];e=[a,s-a-t,t];let i;var o=[1,1,1];let n;var r=1/this.scale,l=2*this.scale/(this.separation*this.scale-1),c=2*this.scale/(this.scale-1),h=[(l-1)*((1+this.separation*this.scale)/(1+2*this.scale-this.separation*this.scale)),(l-1)*((1+this.separation*this.scale)/(1+2*this.scale-this.separation*this.scale)),(l-1)*((1+this.separation*this.scale)/(1+2*this.scale-this.separation*this.scale))],m=[0,c-1,c-1],f=.5*(this.separation-r),d=.5*(this.separation+r),u=.5*(1-r),g=.5*(1+r);for(let M=0;M<this.iterations;M++){var p=Math.abs(e[0]-d)-f,b=Math.abs(e[1]-d)-f,v=Math.abs(e[2]-d)-f,P=Math.max(p,Math.max(b,v)),S=e[0]-r,w=Math.abs(e[1]-g)-u,C=Math.abs(e[2]-g)-u,T=Math.max(S,Math.max(w,C));P<T?(i=P,n=p>Math.max(b,v)?o[0]:b>Math.max(p,v)?o[1]:o[2],e=[l*e[0]-h[0],l*e[1]-h[1],l*e[2]-h[2]],o[0]*=l,o[1]*=l,o[2]*=l):(i=T,n=S>Math.max(w,C)?o[0]:w>Math.max(S,C)?o[1]:o[2],e=[this.scale*e[0]-m[0],c*e[1]-m[1],c*e[2]-m[2]],o[0]*=this.scale,o[1]*=c,o[2]*=c),e=[Math.abs(e[0]),Math.abs(e[1]),Math.abs(e[2])],t=Math.max(Math.max(e[0],e[1]),e[2]),a=Math.min(Math.min(e[0],e[1]),e[2]),s=e[0]+e[1]+e[2],e=[a,s-a-t,t]}return Math.abs(i)/n}changeResolution(resolution=this.imageSize){this.imageSize=Math.max(100,resolution),this.wilson.fullscreen.currentlyFullscreen?[this.imageWidth,this.imageHeight]=Applet.getEqualPixelFullScreen(this.imageSize):(this.imageWidth=this.imageSize,this.imageHeight=this.imageSize),this.wilson.changeCanvasSize(this.imageWidth,this.imageHeight),this.imageWidth>=this.imageHeight?(this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioX,this.imageWidth/this.imageHeight),this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioY,1)):(this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioX,1),this.wilson.gl.uniform1f(this.wilson.uniforms.aspectRatioY,this.imageWidth/this.imageHeight)),this.wilson.gl.uniform1i(this.wilson.uniforms.imageSize,this.imageSize),this.needNewFrame=!0}}export{MengerSponge};