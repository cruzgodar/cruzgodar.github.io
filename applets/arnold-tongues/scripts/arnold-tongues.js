	...
	function use_new_code()
	{
		...
		let frag_shader_source = `
			...
			void main(void)
			{
				...
				{
					theta = theta + z.x - z.y * f(theta);
				}
				
				
				
				float past_thetas[num_iterations];
				...
				for (int i = 1; i <= 10; i++)
				{
					hue += past_thetas[num_iterations - i] * pow(3.0, -float(i));
					
					
					
					for (int j = 1; j < num_iterations - 10; j++)
					{
						if (abs(theta - past_thetas[num_iterations - i - j]) < .01)
						{
							brightness += 1.0 / float(j) * pow(2.0, -float(i));
						}
					}
				}
				...
				gl_FragColor = vec4(hsv2rgb(vec3(hue, 1.0, brightness / 2.0)), 1.0);
			}
		`;
