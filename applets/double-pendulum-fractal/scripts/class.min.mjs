import{buttonAnimationTime,changeOpacity}from"/scripts/src/animation.min.mjs";import{Applet}from"/scripts/src/applets.min.mjs";import{Wilson}from"/scripts/wilson.min.mjs";class DoublePendulumFractal extends Applet{resolution=1e3;dt=.01;drawnFractal=!1;pendulumCanvas=null;lastTimestamp=-1;drawingFractal=!0;wilsonPendulum=null;resolutionPendulum=2e3;lastTimestampPendulum=-1;pendulumCanvasVisible=0;theta1=0;theta2=0;p1=0;p2=0;frame=0;initialTheta1=0;initialTheta2=0;constructor(canvas,pendulumCanvas){super(canvas),this.pendulumCanvas=pendulumCanvas;pendulumCanvas={renderer:"gpu",shader:`
		...
		`),this.wilson.render.createFramebufferTexturePair(),this.wilson.render.createFramebufferTexturePair(),canvas={renderer:"cpu",canvasWidth:this.resolutionPendulum,canvasHeight:this.resolutionPendulum,mousemoveCallback:this.drawPreviewPendulum.bind(this),touchmoveCallback:this.drawPreviewPendulum.bind(this),mousedownCallback:this.startPendulumAnimation.bind(this),touchendCallback:this.startPendulumAnimation.bind(this)};this.wilsonPendulum=new Wilson(this.pendulumCanvas,canvas),this.wilsonPendulum.ctx.lineWidth=this.resolutionPendulum/100,this.wilsonPendulum.ctx.strokeStyle="rgb(127, 0, 255)",this.wilsonPendulum.ctx.fillStyle="rgb(0, 0, 0)",this.wilsonPendulum.draggables.container.addEventListener("mouseleave",()=>{(1===this.pendulumCanvasVisible||this.frame<3)&&(changeOpacity(this.pendulumCanvas,0,buttonAnimationTime),this.pendulumCanvasVisible=0)})}run({resolution}){this.drawnFractal=!1,this.drawingFractal=!0,this.resolution=resolution,this.wilson.changeCanvasSize(this.resolution,this.resolution),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[0].texture),this.wilson.gl.texImage2D(this.wilson.gl.TEXTURE_2D,0,this.wilson.gl.RGBA,this.wilson.canvasWidth,this.wilson.canvasHeight,0,this.wilson.gl.RGBA,this.wilson.gl.FLOAT,null),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[1].texture),this.wilson.gl.texImage2D(this.wilson.gl.TEXTURE_2D,0,this.wilson.gl.RGBA,this.wilson.canvasWidth,this.wilson.canvasHeight,0,this.wilson.gl.RGBA,this.wilson.gl.FLOAT,null),this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[0]),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[0].texture),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,this.wilson.render.framebuffers[0].framebuffer),this.wilson.render.drawFrame(),window.requestAnimationFrame(this.drawFrame.bind(this)),this.drawnFractal=!0}drawFrame(timestamp){var timeElapsed=timestamp-this.lastTimestamp;this.lastTimestamp=timestamp,0!=timeElapsed&&(this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[1]),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,this.wilson.render.framebuffers[1].framebuffer),this.wilson.render.drawFrame(),this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[2]),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[1].texture),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,null),this.wilson.render.drawFrame(),this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[1]),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,this.wilson.render.framebuffers[0].framebuffer),this.wilson.render.drawFrame(),this.wilson.gl.useProgram(this.wilson.render.shaderPrograms[2]),this.wilson.gl.bindTexture(this.wilson.gl.TEXTURE_2D,this.wilson.render.framebuffers[0].texture),this.wilson.gl.bindFramebuffer(this.wilson.gl.FRAMEBUFFER,null),this.wilson.render.drawFrame(),this.drawingFractal)&&!this.animationPaused&&window.requestAnimationFrame(this.drawFrame.bind(this))}drawPreviewPendulum(x,y){this.drawingFractal||(0===this.pendulumCanvasVisible&&this.showPendulumCanvasPreview(),2!==this.pendulumCanvasVisible&&(this.theta1=x*Math.PI,this.theta2=y*Math.PI,this.p1=0,this.p2=0,window.requestAnimationFrame(this.drawFramePendulum.bind(this))))}startPendulumAnimation(){1===this.pendulumCanvasVisible&&(this.initialTheta1=this.theta1,this.initialTheta2=this.theta2,this.p1=0,this.p2=0,this.frame=0,this.showPendulumCanvas())}showPendulumCanvasPreview(){this.drawnFractal&&(this.drawingFractal=!1,changeOpacity(this.pendulumCanvas,.5,buttonAnimationTime),this.pendulumCanvasVisible=1)}showPendulumCanvas(){this.drawnFractal&&(changeOpacity(this.pendulumCanvas,1,buttonAnimationTime),this.pendulumCanvasVisible=2,window.requestAnimationFrame(this.drawFramePendulum.bind(this)))}hidePendulumDrawerCanvas(){this.drawnFractal&&(this.drawingFractal=!0,changeOpacity(this.pendulumCanvas,0,buttonAnimationTime),this.pendulumCanvasVisible=0,window.requestAnimationFrame(this.drawFrame.bind(this)))}drawFramePendulum(timestamp){var hue,z,w,timeElapsed=timestamp-this.lastTimestampPendulum;this.lastTimestampPendulum=timestamp,0!=timeElapsed&&(this.frame++,this.wilsonPendulum.ctx.fillRect(0,0,this.resolutionPendulum,this.resolutionPendulum),timestamp=this.theta1/Math.PI,timeElapsed=this.theta2/Math.PI,z=this.p1/Math.PI,w=this.p2/Math.PI,hue=Math.atan2(timeElapsed,timestamp)/Math.PI+1,timestamp=Math.min(Math.min(50*(timestamp*timestamp+timeElapsed*timeElapsed),5*(1-Math.max(Math.abs(timestamp),Math.abs(timeElapsed)))),1),timeElapsed=.9*((1-this.initialTheta1/(2*Math.PI))*(1-this.initialTheta1/(2*Math.PI))+(1-this.initialTheta2/(2*Math.PI))*(1-this.initialTheta2/(2*Math.PI)))*4,z=Math.min(Math.pow(z*z+w*w,.5)+timeElapsed,1),w=this.wilsonPendulum.utils.hsvToRgb(hue,timestamp,z),this.wilsonPendulum.ctx.strokeStyle=`rgb(${w[0]}, ${w[1]}, ${w[2]})`,this.wilsonPendulum.ctx.beginPath(),this.wilsonPendulum.ctx.moveTo(this.resolutionPendulum/2,this.resolutionPendulum/2),this.wilsonPendulum.ctx.lineTo(this.resolutionPendulum/2+this.resolutionPendulum/6*Math.sin(this.theta1),this.resolutionPendulum/2+this.resolutionPendulum/6*Math.cos(this.theta1)),this.wilsonPendulum.ctx.stroke(),this.wilsonPendulum.ctx.beginPath(),this.wilsonPendulum.ctx.moveTo(this.resolutionPendulum/2+(this.resolutionPendulum/6-this.resolutionPendulum/200)*Math.sin(this.theta1),this.resolutionPendulum/2+(this.resolutionPendulum/6-this.resolutionPendulum/200)*Math.cos(this.theta1)),this.wilsonPendulum.ctx.lineTo(this.resolutionPendulum/2+this.resolutionPendulum/6*Math.sin(this.theta1)+this.resolutionPendulum/6*Math.sin(this.theta2),this.resolutionPendulum/2+this.resolutionPendulum/6*Math.cos(this.theta1)+this.resolutionPendulum/6*Math.cos(this.theta2)),this.wilsonPendulum.ctx.stroke(),2===this.pendulumCanvasVisible)&&(this.updateAngles(),window.requestAnimationFrame(this.drawFramePendulum.bind(this)))}updateAngles(){var dTheta1=6*(2*this.p1-3*Math.cos(this.theta1-this.theta2)*this.p2)/(16-9*Math.pow(Math.cos(this.theta1-this.theta2),2)),dTheta2=6*(8*this.p2-3*Math.cos(this.theta1-this.theta2)*this.p1)/(16-9*Math.pow(Math.cos(this.theta1-this.theta2),2)),dP1=-(dTheta1*dTheta2*Math.sin(this.theta1-this.theta2)+3*Math.sin(this.theta1))/2,dP2=(dTheta1*dTheta2*Math.sin(this.theta1-this.theta2)-Math.sin(this.theta2))/2;this.theta1+=dTheta1*this.dt*2.5,this.theta2+=dTheta2*this.dt*2.5,this.p1+=dP1*this.dt*2.5,this.p2+=dP2*this.dt*2.5,this.theta1>=Math.PI?this.theta1-=2*Math.PI:this.theta1<-Math.PI&&(this.theta1+=2*Math.PI),this.theta2>=Math.PI?this.theta2-=2*Math.PI:this.theta2<-Math.PI&&(this.theta2+=2*Math.PI)}}export{DoublePendulumFractal};