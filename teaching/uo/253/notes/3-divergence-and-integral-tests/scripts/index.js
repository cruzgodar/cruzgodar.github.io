	...
	Page.Load.getDesmosData = () =>
	{
		const data =
		{
			...
			"riemann-sum-2":
			{
				...
				expressions:
				[
					{latex: String.raw`f(x) = \frac{1}{x + 4}`, color: DESMOS_PURPLE},
					...
					{latex: String.raw`0 \leq y \leq f(R - 1) \{ L - 1 \leq x \leq R - 1 \}`, color: DESMOS_BLUE, secret: true},
					{latex: String.raw`x = L - 1 \{ 0 \leq y \leq f(R - 1) \}`, color: DESMOS_BLUE, secret: true},
					{latex: String.raw`x = R - 1 \{ 0 \leq y \leq f(R - 1) \}`, color: DESMOS_BLUE, secret: true},
