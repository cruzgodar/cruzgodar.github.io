...
\begin{document}\Large Name: \rule{2in}{0.15mm} \hfill Homework 6 | Math 342 | Cruz Godar \vspace{4pt}  \large

\textit{Due Wednesday of Week 7 at the start of class}

Complete the following problems and submit them as a pdf to Canvas. 8 points are awarded for thoroughly attempting every problem, and I'll select three problems to grade on correctness for 4 points each. Enough work should be shown that there is no question about the mathematical process used to obtain your answers.

<button class="text-button" type="button" id="download-homework-6-pdf-button" tabindex="-1"></button><button class="text-button" type="button" id="download-homework-6-tex-button" tabindex="-1"></button>
\section{Section 5}

In problems 1--4, find the least-squares solution $\vec{x}\hspace{0.1em}'$ to the matrix equation $A\vec{x} = \vec{b}$ and the distance between $A\vec{x}\hspace{0.1em}'$ and $\vec{b}$. Also determine if that least-squares solution is unique.
...
\begin{enumerate}
	...
	\item $\displaystyle \left< f, g \right> = \int_0^1 |f(x) + g(x)|\,\mathrm{d} x.$

	\item $\displaystyle \left< f, g \right> = \int_0^1 (f(x)g(x))^2\,\mathrm{d} x.$

	\item $\displaystyle \left< f, g \right> = \int_0^1 f(g(x))\,\mathrm{d} x.$

	\item $\displaystyle \left< f, g \right> = \int_{1/4}^{3/4} f(x)g(x)\,\mathrm{d} x.$
