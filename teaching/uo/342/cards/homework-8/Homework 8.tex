...
\begin{document}\Large Name: \rule{2in}{0.15mm} \hfill Homework 8 | Math 342 | Cruz Godar \vspace{4pt}  \large

\textit{Due Wednesday of Week 10 at the start of class}

Complete the following problems and submit them as a pdf to Canvas. 8 points are awarded for thoroughly attempting every problem, and I'll select three problems to grade on correctness for 4 points each. Enough work should be shown that there is no question about the mathematical process used to obtain your answers.

<button class="text-button" type="button" id="download-homework-8-pdf-button" tabindex="-1"></button><button class="text-button" type="button" id="download-homework-8-tex-button" tabindex="-1"></button>
~\\
...
\begin{enumerate}
	...
	\item If the eigenvalues of $A$ are distinct, find the general solution to the system of differential equations $\vec{x}\hspace{0.1em}' = A\vec{x}$.
...
\begin{enumerate}
	...
	\item Find the orthogonal decomposition of the vector $\vec{v}$ as $\vec{v} = \vec{x} + \vec{x}\hspace{0.1em}'$ for $\vec{x} \in X$ and $\vec{x}\hspace{0.1em}' \in X^\perp$.
...
11. (optional) $V = \operatorname{span}\left\{ 1, \sin(x), \cos(x) \right\}$ with $\left< f, g \right> = \int_0^1 f(x)g(x)\,\mathrm{d} x$, $X$ is the subspace of functions $f$ with $f'(0) = 0$, and $\vec{v} = \sin(x) + 2\cos(x) - 1$.

12. (optional; more recommended than the previous problem) $V = \operatorname{span}\left\{ 1, \cos(x) \right\}$ with $\left< f, g \right> = \int_0^{\pi/2} f(x)g(x)\,\mathrm{d} x$, $X = \operatorname{span}\{1\}$, and $\vec{v} = \cos(x) - 2$.
