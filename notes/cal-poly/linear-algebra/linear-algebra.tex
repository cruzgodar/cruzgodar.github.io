...
\begin{Def}
	...
	Let $V$ be a vector space and $U$ and $V$ subspaces of $V$. The \textbf{sum} of $U$ and $V$ is $U+W = \{u+w \mid u \in U, w \in W\}$. If each element of $V$ can be expressed uniquely as an element of $U+W$, we say that $U+W$ is a \textbf{direct sum}, and we write $U \oplus W$.
...
\begin{Def}
	...
	The \textbf{span} of $v_1, ..., v_n \in V$ is the set $\Span \{v_1, ..., v_n\} = \{c_1 v_1 + \cdots + c_n v_n \mid c_i \in k\}$. A set of vectors $\{v_1, ..., v_n\} \subseteq V$ \textbf{spans} $V$ if $\Span \{v_1, ..., v_n\} = V$.
...
\begin{Def}
	...
	Let $T \in \mathcal{L}(V,W)$. The \textbf{null space} of $T$ is the set $\Null T = \{x \in V \mid Tx = 0\}$, and the \textbf{range} of $T$ is the set $\Range T = \{Tv \mid v \in V\}$.
...
\begin{Def}
	...
	Let $U \subseteq V$ (not necessarily a subspace). The \textbf{annihilator} of $U$ is the set $U^0 = \{\varphi \in V' \mid \varphi u = 0\ \textnormal{for all}\ u \in U\}$.
...
\begin{Def}
	...
	Let $U \subseteq V$. The \textbf{orthogonal complement} to $U$ is the set $U^\perp = \{v \in V \mid \left< u, v \right> = 0\ \textnormal{for all}\ u \in U\}$.
...
\begin{Theorem}
	...
	$$
		\mathcal{A} = \{f : (\mathbb{R}^n)^n \longrightarrow \mathbb{R} \mid f(a_1, ..., a_i, ..., a_j, ..., a_n) = -f(a_1, ..., a_j, ..., a_i, ..., a_n), \\ \ f \textnormal{ is coordinate-wise linear} \}
	$$
